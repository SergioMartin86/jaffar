  ...
  if (Char.alive >= 0) {
    ...
    if (char_action == actions_5_bumped ||
      ...
      control_guard_inactive();
               ...
               (char_frame >= frame_50_turn &&
                char_frame < 53)  // end of turning
    ) {
      control_standing();
      ...
      control_startrun();
    } else if (char_frame >= frame_67_start_jump_up_1 &&
               char_frame < frame_70_jumphang) {  // start jump up
      control_jumpup();
    } else if (char_frame < 15) {  // running
      control_running();
    } else if (char_frame >= frame_87_hanging_1 &&
               char_frame < 100) {  // hanging
      control_hanging();
...
void __pascal far control_crouched() {
  if (need_level1_music != 0 &&
      current_level == /*1*/ custom->intro_music_level) {
    ...
    if (!check_sound_playing()) {
      if (need_level1_music == 1) {
        ...
        if (!replaying)  // during replays, crouch immobilization gets cancelled
                         // in do_replay_move()
          need_level1_music = 0;
      }
    }
      ...
      if (var_2 >= -10 && var_2 < 90) {
        ...
        if ((word)var_2 < (word)-6) {
          if (Opp.charid == charid_1_shadow &&
              (Opp.action == actions_3_in_midair ||
               (Opp.frame >= frame_107_fall_land_1 && Opp.frame < 118))) {
            offguard = 0;
          } else {
            draw_sword();
            return;
...
void __pascal far up_pressed() {
  ...
  if (get_tile_at_char() == tiles_16_level_door_left)
    ...
    leveldoor_tilepos = curr_tilepos;
  if ((leveldoor_tilepos != -1) && level.start_room != drawn_room &&
      (fixes->fix_exit_door ? curr_room_modif[leveldoor_tilepos] >=
                                  42  // this door must be fully open
                            : leveldoor_open)) {
    go_up_leveldoor();
  } else {
    if (control_x < 0) {
...
void __pascal far down_pressed() {
  ...
  if (!tile_is_floor(get_tile_infrontof_char()) &&
    ...
    if (!tile_is_floor(get_tile_behind_char()) &&
      ...
      get_tile_at_char();
      if (can_grab() && (Char.direction >= dir_0_right ||
...
void __pascal far control_turning() {
  if (control_shift >= 0 && control_x < 0 && control_y >= 0) {
    seqtbl_offset_char(
        seq_43_start_run_after_turn);  // start run and run (after turning)
  }

  // Added:
  // When using a joystick, the kid may sometimes jump/duck/turn unintendedly
  // after turning around. To prevent this: clear the remembered controls, so
  // that if the stick has already moved to another/neutral position, the kid
  // will not jump, duck, or turn again.
  if (is_joyst_mode) {
...
void __pascal far control_running() {
  if (control_x == 0 &&
      (Char.frame == frame_7_run || Char.frame == frame_11_run)) {
...
int __pascal far check_get_item() {
  if (get_tile_at_char() == tiles_10_potion || curr_tile2 == tiles_22_sword) {
    if (!tile_is_floor(get_tile_behind_char())) {
      return 0;
    }
...
void __pascal far grab_up_no_floor_behind() {
  ...
  seqtbl_offset_char(
      seq_16_jump_up_and_grab);  // jump up and grab (no floor behind)
}
...
void __pascal far jump_up() {
  ...
  get_tile(Char.room, get_tile_div_mod(back_delta_x(0) + dx_weight() - 6),
           Char.curr_row - 1);
  if (curr_tile2 != tiles_20_wall && !tile_is_floor(curr_tile2)) {
    seqtbl_offset_char(
        seq_28_jump_up_with_nothing_above);  // jump up with nothing above
  } else {
    seqtbl_offset_char(
        seq_14_jump_up_into_ceiling);  // jump up with wall or floor above
  }
}
  ...
  if (Char.alive < 0) {
    if (grab_timer == 0 && control_y < 0) {
      ...
      if (Char.action != actions_6_hang_straight &&
        ...
        seqtbl_offset_char(
            seq_25_hang_against_wall);  // hang against wall (straight)
      } else {
...
void __pascal far can_climb_up() {
  ...
  if (((curr_tile2 == tiles_13_mirror || curr_tile2 == tiles_18_chomper) &&
      ...
      (curr_tile2 == tiles_4_gate && Char.direction != dir_0_right &&
       curr_room_modif[curr_tilepos] >> 2 < 6)) {
    seq_id =
        seq_73_climb_up_to_closed_gate;  // climb up to closed gate and down
  }
  seqtbl_offset_char(seq_id);
}
...
void __pascal far hang_fall() {
  ...
  if (!tile_is_floor(get_tile_behind_char()) &&
    ...
    seqtbl_offset_char(
        seq_23_release_ledge_and_fall);  // release ledge and fall
  } else {
    if (get_tile_at_char() == tiles_20_wall ||
        (Char.direction < dir_0_right &&
         (  // looking left
...
void __pascal far grab_up_with_floor_behind() {
  ...
  // The global variable edge_type (which we need!) gets set as a side effect of
  // get_edge_distance()
...
#define JUMP_STRAIGHT_CONDITION \
  distance < 4 && edge_distance < 4 && edge_type != 1

  if (JUMP_STRAIGHT_CONDITION) {
    Char.x = char_dx_forward(distance);
    seqtbl_offset_char(
        seq_8_jump_up_and_grab_straight);  // jump up and grab (when?)
  } else {
    Char.x = char_dx_forward(distance - 4);
    seqtbl_offset_char(seq_24_jump_up_and_grab_forward);  // jump up and grab
                                                          // (with floor behind)
  }
}
...
void __pascal far back_with_sword() {
  short frame;
  frame = Char.frame;
  if (frame == frame_158_stand_with_sword ||
      frame == frame_170_stand_with_sword ||
      frame == frame_171_stand_with_sword) {
...
void __pascal far forward_with_sword() {
  ...
  if (frame == frame_158_stand_with_sword ||
    ...
    if (Char.charid != charid_0_kid) {
      seqtbl_offset_char(
          seq_56_guard_forward_with_sword);  // forward with sword (Guard)
    } else {
      seqtbl_offset_char(
          seq_86_forward_with_sword);  // forward with sword (Kid)
    }
  }
}
    ...
    if (get_tile_at_char() == tiles_11_loose || can_guard_see_kid >= 2) {
      ...
      if ((word)distance < (word)90) {
        ...
        if ((word)distance < (word)-4) {
          seqtbl_offset_char(seq_60_turn_with_sword);  // turn with sword (after
                                                       // switching places)
          return;
        } else {
          swordfight();
    ...
    } /*else*/
    {
      ...
      if (Char.charid < charid_2_guard) {
        ...
        if (Char.frame == frame_171_stand_with_sword) {
          ...
          seqtbl_offset_char(
              seq_92_put_sword_away);  // put sword away (Guard died)
        }
      } else {
        swordfight();
      }
...
void __pascal far swordfight() {
  ...
  if (frame == frame_161_parry && control_shift2 >= 0) {
    seqtbl_offset_char(
        seq_57_back_with_sword);  // back with sword (when parrying)
    return;
  ...
  if (control_down < 0) {
    if (frame == frame_158_stand_with_sword ||
      ...
      if (charid == charid_0_kid) {
        ...
        holding_sword = 0;
        seq_id =
            seq_93_put_sword_away_fast;  // put sword away fast (down pressed)
      } else if (charid == charid_1_shadow) {
        seq_id = seq_92_put_sword_away;  // put sword away
      } else {
        seq_id = seq_87_guard_become_inactive;  // stand inactive (when Kid
                                                // leaves sight)
      }
      seqtbl_offset_char(seq_id);
    }
...
void __pascal far parry() {
  ...
  var_6 = 0;
  if (char_frame == frame_158_stand_with_sword ||  // stand with sword
    ...
    if (char_opp_dist() >= 32 && char_charid != charid_0_kid) {
      ...
      if (opp_frame != frame_151_strike_1 && opp_frame != frame_152_strike_2 &&
