...
const word replay_format_class =
    0;  // unique number associated with this SDLPoP implementation / fork
const char* implementation_name = "SDLPoP v" SDLPOP_VERSION;

#define REPLAY_FORMAT_CURR_VERSION \
  102  // current version number of the replay format
#define REPLAY_FORMAT_MIN_VERSION \
  101  // SDLPoP will open replays with this version number and higher
#define REPLAY_FORMAT_DEPRECATION_NUMBER \
  1  // SDLPoP won't open replays with a higher deprecation number

#define MAX_REPLAY_DURATION 345600  // 8 hours: 720 * 60 * 8 ticks
byte moves[MAX_REPLAY_DURATION] = {
    0};  // static memory for now because it is easier (should this be dynamic?)

char replay_levelset_name[POP_MAX_PATH];
char stored_levelset_name[POP_MAX_PATH];

// 1-byte structure representing which controls were active at a particular game
// tick
...
// information needed to keep track of all listed replay files, and to sort them
...
int read_replay_header(replay_header_type* header, FILE* fp,
                       char* error_message) {
  // Explicitly go to the beginning, because the current filepos might be
  // nonzero.
  fseek(fp, 0, SEEK_SET);
  ...
  // read the unique number associated with this SDLPoP implementation / fork
  // (for normal SDLPoP: 0)
  word class;
  ...
  // creation time (seconds since 1970) is embedded in the format, but not used
  // in SDLPoP right now
  ...
  if (class != replay_format_class) {
    // incompatible, replay format is associated with a different implementation
    // of SDLPoP
    if (error_message != NULL) {
      snprintf_check(error_message, REPLAY_HEADER_ERROR_MESSAGE_MAX,
                     "replay created with \"%s\"...\nIncompatible replay class "
                     "identifier! (expected %d, found %d)",
  ...
  if (version_number < REPLAY_FORMAT_MIN_VERSION) {
    ...
    if (error_message != NULL) {
      snprintf_check(error_message, REPLAY_HEADER_ERROR_MESSAGE_MAX,
                     "replay created with \"%s\"...\nReplay format version too "
                     "old! (minimum %d, found %d)",
                     header->implementation_name, REPLAY_FORMAT_MIN_VERSION,
                     version_number);
    }
    return 0;
  }
  ...
  if (deprecation_number > REPLAY_FORMAT_DEPRECATION_NUMBER) {
    ...
    if (error_message != NULL) {
      snprintf_check(error_message, REPLAY_HEADER_ERROR_MESSAGE_MAX,
                     "replay created with \"%s\"...\nReplay deprecation number "
                     "too new! (max %d, found %d)",
                     header->implementation_name,
                     REPLAY_FORMAT_DEPRECATION_NUMBER, deprecation_number);
    }
    return 0;
  }
  ...
  if (is_validate_mode) {
    ...
    if (!is_replay_info_printed) {
      printf("\nReplay created with %s.\n", header->implementation_name);
      printf(
          "Format: class identifier %d, version number %d, deprecation number "
          "%d.\n",
...
size_t max_replay_files =
    128;  // initially, may grow if there are > 128 replay files found
replay_info_type* replay_list = NULL;

// Compare function -- for qsort() in list_replay_files() below
// Compares creation dates of replays, so they can be loaded in reverse creation
// order (newest first)
static int compare_replay_creation_time(const void* a, const void* b) {
  return (int)difftime(((replay_info_type*)b)->creation_time,
                       ((replay_info_type*)a)->creation_time);
}

void list_replay_files() {
  if (replay_list == NULL) {
    // need to allocate enough memory to store info about all replay files in
    // the directory
    replay_list = malloc(
        max_replay_files *
        sizeof(replay_info_type));  // will realloc() later if > 256 files exist
  }

  num_replay_files = 0;

  directory_listing_type* directory_listing =
      create_directory_listing_and_find_first_file(replays_folder, "p1r");
  ...
  do {
    ...
    if (num_replay_files > max_replay_files) {
      ...
      replay_list =
          realloc(replay_list, max_replay_files * sizeof(replay_info_type));
    }
    replay_info_type* replay_info =
        &replay_list[num_replay_files - 1];  // current replay file
    memset(replay_info, 0, sizeof(replay_info_type));
    // store the filename of the replay
    snprintf_check(
        replay_info->filename, POP_MAX_PATH, "%s/%s", replays_folder,
  ...
  if (num_replay_files > 1) {
    ...
    qsort(replay_list, (size_t)num_replay_files, sizeof(replay_info_type),
...
// (double-clicked, dragged onto .exe, etc.)
void start_with_replay_file(const char* filename) {
  if (open_replay_file(filename)) {
    ...
    if (!ok) {
      ...
      if (is_validate_mode)  // Validating replays is cmd-line only, so, no
                             // sense continuing from here.
        exit(0);

      SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "SDLPoP", error_message,
                               NULL);
      return;
    }
    if (header.uses_custom_levelset) {
      strncpy(replay_levelset_name, header.levelset_name,
              sizeof(replay_levelset_name));  // use the replays's levelset
    }
    rewind(replay_fp);  // replay file is still open and will be read in
                        // load_replay() later
    need_start_replay =
        1;  // will later call start_replay(), from init_record_replay()
  }
}

// The functions options_process_* below each process (read/write) a section of
// options variables (using SDL_RWops) This is I/O for the *binary*
// representation of the relevant options - this gets saved as part of a replay.

typedef int rw_process_func_type(SDL_RWops* rw, void* data, size_t data_size);
typedef void process_options_section_func_type(
    SDL_RWops* rw, rw_process_func_type process_func);

#define process(x) \
  if (!process_func(rw, &(x), sizeof(x))) return

void options_process_features(SDL_RWops* rw,
...
void options_process_enhancements(SDL_RWops* rw,
                                  rw_process_func_type process_func) {
...
void options_process_custom_general(SDL_RWops* rw,
                                    rw_process_func_type process_func) {
...
void options_process_custom_per_level(SDL_RWops* rw,
                                      rw_process_func_type process_func) {
...
// struct for keeping track of both the normal and the replay options (which we
// want to easily switch between) (separately for each 'section', so adding
// future options becomes easy without messing up the format!)
typedef struct replay_options_section_type {
  dword data_size;
  byte replay_data[POP_MAX_OPTIONS_SIZE];  // binary representation of the
                                           // options that are active during the
                                           // replay
  byte stored_data[POP_MAX_OPTIONS_SIZE];  // normal options are restored from
                                           // this, after the replay is finished
...
// output the current options to a memory buffer (e.g. to remember them before a
// replay is loaded)
size_t save_options_to_buffer(
    void* options_buffer, size_t max_size,
    process_options_section_func_type* process_section_func) {
...
// restore the options from a memory buffer (e.g. reapply the original options
// after a replay is finished)
void load_options_from_buffer(
    void* options_buffer, size_t options_size,
    process_options_section_func_type* process_section_func) {
...
void replay_restore_level() {
  // Need to restore the savestate at the right time (just before the first room
  // of the level is drawn). Otherwise, for "on-the-fly" recordings, the screen
  // will visibly "jump" to the replay savestate. This only needs to happen at
  // the very beginning of the replay (curr_tick == 0)
...
int savestate_to_buffer() {
  int ok = 0;
  if (savestate_buffer == NULL) savestate_buffer = malloc(MAX_SAVESTATE_SIZE);
...
void reload_resources() {
  // the replay's levelset might use different sounds, so we need to free and
  // reload sounds
  free_all_sounds();
  load_all_sounds();
  free_all_chtabs_from(id_chtab_0_sword);
  // chtabs 3 and higher will be freed/reloaded in load_lev_spr() (called by
  // restore_room_after_quick_load()) However, chtabs 0-2 are usually not freed
  // at all (they are loaded only once, in init_game_main()) So we should reload
  // them manually (PRINCE.DAT and KID.DAT may still have been modified after
  // all!)
...
int restore_savestate_from_buffer() {
  int ok = 0;
  savestate_offset = 0;
  // This condition should be checked in process_load_from_buffer() instead of
  // here.
...
void start_recording() {
  curr_tick = 0;
  recording = 1;  // further set-up is done in add_replay_move, on the first
                  // gameplay tick
}
...
void apply_replay_options() {
  ...
  for (int i = 0; i < COUNT(replay_options_sections); ++i) {
    save_options_to_buffer(replay_options_sections[i].stored_data,
                           POP_MAX_OPTIONS_SIZE,
                           replay_options_sections[i].section_func);
  }

  // apply the options from the memory buffer (max. replay_options_size bytes
  // will be read)
  for (int i = 0; i < COUNT(replay_options_sections); ++i) {
    load_options_from_buffer(replay_options_sections[i].replay_data,
                             replay_options_sections[i].data_size,
                             replay_options_sections[i].section_func);
  }
  ...
  load_mod_options();  // Load resources from the correct places if there is a
                       // mod name in the replay file. This also prevents
                       // unwanted switching to PC Speaker mode.
  reload_resources();
}
...
void restore_normal_options() {
  ...
  for (int i = 0; i < COUNT(replay_options_sections); ++i) {
    load_options_from_buffer(replay_options_sections[i].stored_data,
                             POP_MAX_OPTIONS_SIZE,
                             replay_options_sections[i].section_func);
  }
...
static void print_remaining_time() {
  if (rem_min > 0) {
    printf("Remaining time: %d min, %d sec, %d ticks. ", rem_min - 1,
           rem_tick / 12, rem_tick % 12);
  } else {
    printf("Elapsed time:   %d min, %d sec, %d ticks. ", -(rem_min + 1),
           (719 - rem_tick) / 12, (719 - rem_tick) % 12);
  }
  printf("(rem_min=%d, rem_tick=%d)\n", rem_min, rem_tick);
}

void start_replay() {
  stop_sounds();  // Don't crash if the intro music is interrupted by Tab in PC
                  // Speaker mode.
  if (!enable_replay) return;
  need_start_replay = 0;
  if (!is_validate_mode) {
    list_replay_files();
    // If the replay was started from a file given in the command line, we don't
    // care if there are no replay files in the replay folder.
...
void end_replay() {
  if (!is_validate_mode) {
    ...
    if (num_replay_ticks != curr_tick) {
      printf(
          "WARNING: Play duration does not match replay length. (%d ticks)\n",
          num_replay_ticks);
    } else {
...
void do_replay_move() {
  ...
  if (current_level == next_level) {
    ...
    // Ignore shift if the kid is dead: restart moves are hard-coded as a
    // 'special move'.
    ...
    //    if (curr_tick > 5 ) printf("rem_tick: %d\t curr_tick: %d\tlast 5
    //    moves: %d, %d, %d, %d, %d\n", rem_tick, curr_tick,
    //                               moves[curr_tick-4], moves[curr_tick-3],
    //                               moves[curr_tick-2], moves[curr_tick-1],
    //                               moves[curr_tick]);
    ++curr_tick;
  }
}
...
int save_recorded_replay() {
  ...
  current_target_surface = onscreen_surface_;
  need_full_redraw = 1;  // lazy: instead of neatly restoring the dialog peel,
  ...
  do {
    input_length =
        input_str(&input_rect, input_filename, 64, "", 0, 0, color, bgcolor);
  ...
  snprintf_check(full_filename, sizeof(full_filename), "%s/%s.p1r",
                 replays_folder, input_filename);
  ...
  // NOTE: We currently overwrite the replay file if it exists already. Maybe
  // warn / ask for confirmation??

  replay_fp = fopen(full_filename, "wb");
  if (replay_fp != NULL) {
    fwrite(replay_magic_number, COUNT(replay_magic_number), 1,
           replay_fp);  // magic number "P1R"
    ...
    // levelset_name
    putc(strnlen(levelset_name, UINT8_MAX),
         replay_fp);  // length of the levelset name (is zero for original
                      // levels)
    ...
    for (int i = 0; i < COUNT(replay_options_sections); ++i) {
      dword section_size =
          save_options_to_buffer(temp_options, sizeof(temp_options),
                                 replay_options_sections[i].section_func);
...
void replay_cycle() {
  ...
  skipping_replay = 0;
  stop_sounds();
  if (current_replay_number ==
          -1 /* opened .P1R file directly, so cycling is disabled */
      || !open_next_replay_file() || !load_replay()) {
    // there is no replay to be cycled to after the current one --> restart the
    // game
    replaying = 0;
    restore_normal_options();
    start_game();
...
int load_replay() {
  ...
  if (replay_fp != NULL && savestate_buffer != NULL) {
    ...
    memcpy(replay_levelset_name, header.levelset_name,
           sizeof(header.levelset_name));
    ...
    for (int i = 0; i < COUNT(replay_options_sections); ++i) {
      ...
      fread_check(replay_options_sections[i].replay_data, section_size, 1,
                  replay_fp);
...
void key_press_while_replaying(int* key_ptr) {
  ...
  switch (key) {
    case 0:  // 'no key pressed'
      break;
    default:
      // cannot manually do most stuff during a replay, so cancel the pressed
      // key...
      *key_ptr =
          1;  // don't set to zero (we would be unable to unpause a replay
              // because all keys are ignored) (1 is not in use as a scancode,
