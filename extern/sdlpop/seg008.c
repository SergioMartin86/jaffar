...
// seg008:02FE
int __pascal far get_tile_to_draw(int room, int column, int row,
                                  byte* ptr_tiletype, byte* ptr_modifier,
                                  byte tile_room0) {
...
void __pascal far load_curr_and_left_tile() {
  ...
  if (drawn_row == 2) {
    tiletype = custom->drawn_tile_top_level_edge;  // floor at top of level
                                                   // (default: tiles_1_floor)
  }
  get_tile_to_draw(drawn_room, drawn_col, drawn_row, &curr_tile, &curr_modifier,
                   tiletype);
  get_tile_to_draw(drawn_room, drawn_col - 1, drawn_row, &tile_left,
                   &modifier_left, tiletype);
...
void __pascal far load_leftroom() {
  ...
  for (row = 0; row < 3; ++row) {
    // wall at left of level (drawn_tile_left_level_edge), default:
    // tiles_20_wall
    get_tile_to_draw(room_L, 9, row, &leftroom_[row].tiletype,
                     &leftroom_[row].modifier,
                     custom->drawn_tile_left_level_edge);
  }
}
...
void __pascal far load_rowbelow() {
  ...
  for (column = 1; column < 10; ++column) {
    get_tile_to_draw(room, column - 1, row_below,
                     &row_below_left_[column].tiletype,
                     &row_below_left_[column].modifier, tiles_0_empty);
  }
  get_room_address(room_left);
  // wall at left of level
  get_tile_to_draw(room_left, 9, row_below, &row_below_left_[0].tiletype,
                   &row_below_left_[0].modifier, tiles_20_wall);
  get_room_address(drawn_room);
}
...
void __pascal far draw_tile_floorright() {
  ...
  add_backtable(id_chtab_6_environment, 42 /*floor right part*/, draw_xh, 0,
                tile_table[tiles_1_floor].right_y + draw_main_y,
                blitters_9_black, 1);
}

// seg008:053A
int __pascal far can_see_bottomleft() {
  return curr_tile == tiles_0_empty || curr_tile == tiles_9_bigpillar_top ||
         curr_tile == tiles_12_doortop || curr_tile == tiles_26_lattice_down;
}
...
void __pascal far draw_tile_topright() {
  ...
  if (tiletype == tiles_7_doortop_with_floor || tiletype == tiles_12_doortop) {
    ...
    add_backtable(id_chtab_6_environment,
                  doortop_fram_top[row_below_left_[drawn_col].modifier],
                  draw_xh, 0, draw_bottom_y, blitters_2_or, 0);
  } else if (tiletype == tiles_20_wall) {
    add_backtable(id_chtab_7_environmentwall, 2, draw_xh, 0, draw_bottom_y,
                  blitters_2_or, 0);
  } else {
    add_backtable(id_chtab_6_environment, tile_table[tiletype].topright_id,
                  draw_xh, 0, draw_bottom_y, blitters_2_or, 0);
  }
}
...
void __pascal far draw_tile_anim_topright() {
  word modifier;
  if ((curr_tile == tiles_0_empty || curr_tile == tiles_9_bigpillar_top ||
       curr_tile == tiles_12_doortop) &&
      row_below_left_[drawn_col].tiletype == tiles_4_gate) {
    add_backtable(id_chtab_6_environment, 68 /*gate top mask*/, draw_xh, 0,
                  draw_bottom_y, blitters_40h_mono, 0);
    modifier = row_below_left_[drawn_col].modifier;
    if (modifier > 188) modifier = 188;
    add_backtable(id_chtab_6_environment, door_fram_top[(modifier >> 2) % 8],
                  draw_xh, 0, draw_bottom_y, blitters_2_or, 0);
  }
}
      ...
      if (id) {
        if (tile_left == tiles_5_stuck) {
          ...
          if (curr_tile == tiles_0_empty || curr_tile == tiles_5_stuck ||
              !tile_is_floor(curr_tile)) {
            id = 42; /*floor B*/
          }
        } else {
          blit = blitters_2_or;
        }
        add_backtable(id_chtab_6_environment, id, draw_xh, 0,
                      tile_table[tile_left].right_y + draw_main_y, blit, 0);
      }
      if (custom->tbl_level_type[current_level] != 0) {
        add_backtable(id_chtab_6_environment, tile_table[tile_left].stripe_id,
                      draw_xh, 0, draw_main_y - 27, blitters_2_or, 0);
      }
      if (tile_left == tiles_19_torch ||
          tile_left == tiles_30_torch_with_debris) {
        add_backtable(id_chtab_6_environment, 146 /*torch base*/, draw_xh, 0,
                      draw_bottom_y - 28, blitters_0_no_transp, 0);
      }
      break;
    case tiles_0_empty:
      if (modifier_left > 3) return;
      add_backtable(id_chtab_6_environment, blueline_fram1[modifier_left],
                    draw_xh, 0, blueline_fram_y[modifier_left] + draw_main_y,
                    blitters_2_or, 0);
      break;
    case tiles_1_floor:
      ptr_add_table(id_chtab_6_environment, 42 /*floor B*/, draw_xh, 0,
                    tile_table[tile_left].right_y + draw_main_y,
                    blitters_10h_transp, 0);
      var_2 = modifier_left;
      if (var_2 > 3) var_2 = 0;
      if (var_2 == !!custom->tbl_level_type[current_level]) return;
      add_backtable(id_chtab_6_environment, blueline_fram3[var_2], draw_xh, 0,
                    draw_main_y - 20, blitters_0_no_transp, 0);
      break;
    case tiles_7_doortop_with_floor:
    case tiles_12_doortop:
      if (custom->tbl_level_type[current_level] == 0) return;
      add_backtable(id_chtab_6_environment, doortop_fram_bot[modifier_left],
                    draw_xh, 0, tile_table[tile_left].right_y + draw_main_y,
                    blitters_2_or, 0);
      break;
    case tiles_20_wall:
      if (custom->tbl_level_type[current_level] &&
          (modifier_left & 0x80) == 0) {
        add_backtable(id_chtab_6_environment, 84 /*wall stripe*/, draw_xh + 3,
                      0, draw_main_y - 27, blitters_0_no_transp, 0);
      }
      add_backtable(id_chtab_7_environmentwall, 1, draw_xh, 0,
                    tile_table[tile_left].right_y + draw_main_y, blitters_2_or,
                    0);
      break;
  }
}
...
void __pascal far draw_tile_anim_right() {
  switch (tile_left) {
    case tiles_2_spike:
      add_backtable(id_chtab_6_environment,
                    spikes_fram_right[get_spike_frame(modifier_left)], draw_xh,
                    0, draw_main_y - 7, blitters_10h_transp, 0);
      break;
    case tiles_4_gate:
      draw_gate_back();
      break;
    case tiles_11_loose:
      add_backtable(id_chtab_6_environment,
                    loose_fram_right[get_loose_frame(modifier_left)], draw_xh,
                    0, draw_bottom_y - 1, blitters_2_or, 0);
      break;
    ...
    case tiles_30_torch_with_debris:
      if (modifier_left < 9) {
        ...
        add_backtable(id_chtab_1_flameswordpotion, modifier_left + 1,
                      draw_xh + 1, 0, draw_main_y - 40, blit, 0);
      }
      break;
  }
}
...
void __pascal far draw_tile_bottom(word arg_0) {
  ...
  switch (curr_tile) {
    case tiles_20_wall:
      if (custom->tbl_level_type[current_level] == 0 ||
          custom->enable_wda_in_palace || graphics_mode != gmMcgaVga) {
  ...
  if (ptr_add_table(chtab_id, id, draw_xh, 0, draw_bottom_y, blit, 0) &&
      arg_0) {
    add_foretable(chtab_id, id, draw_xh, 0, draw_bottom_y, blit, 0);
  }
  if (chtab_id == id_chtab_7_environmentwall && graphics_mode != gmCga &&
      graphics_mode != gmHgaHerc) {
    wall_pattern(0, 0);
  }
}

const byte loose_fram_bottom[] = {43, 73, 43, 74, 74, 43,
                                  43, 43, 74, 74, 74, 0};
...
void __pascal far draw_loose(int arg_0) {
  ...
  if (curr_tile == tiles_11_loose) {
    ...
    add_backtable(id_chtab_6_environment, id, draw_xh, 0, draw_bottom_y,
                  blitters_0_no_transp, 0);
    add_foretable(id_chtab_6_environment, id, draw_xh, 0, draw_bottom_y,
                  blitters_0_no_transp, 0);
  }
}
...
void __pascal far draw_tile_base() {
  ...
  } else if (curr_tile == tiles_15_opener && tile_left == tiles_0_empty &&
             custom->tbl_level_type[current_level] == 0) {
    id = 148;  // left half of open button with no floor to the left
  } else {
    id = tile_table[curr_tile].base_id;
  }
  ptr_add_table(id_chtab_6_environment, id, draw_xh, 0,
                tile_table[curr_tile].base_y + ybottom, blitters_10h_transp, 0);
}
...
void __pascal far draw_tile_anim() {
  ...
  switch (curr_tile) {
    case tiles_2_spike:
      ptr_add_table(id_chtab_6_environment,
                    spikes_fram_left[get_spike_frame(curr_modifier)], draw_xh,
                    0, draw_main_y - 2, blitters_10h_transp, 0);
      break;
    case tiles_10_potion:
      ...
      add_backtable(id_chtab_1_flameswordpotion, 23 /*bubble mask*/,
                    draw_xh + 3, 1, draw_main_y - (pot_size << 2) - 14,
                    blitters_40h_mono, 0);
      add_foretable(id_chtab_1_flameswordpotion,
                    potion_fram_bubb[curr_modifier & 0x7], draw_xh + 3, 1,
                    draw_main_y - (pot_size << 2) - 14,
                    color + blitters_40h_mono, 0);
      break;
    case tiles_22_sword:
      add_midtable(id_chtab_1_flameswordpotion, (curr_modifier == 1) + 10,
                   draw_xh, 0, draw_main_y - 3, blitters_10h_transp,
                   curr_modifier == 1);
      break;
    case tiles_18_chomper:
      var_4 = chomper_fram1[MIN(curr_modifier & 0x7F, 6)];
      add_backtable(id_chtab_6_environment, chomper_fram_bot[var_4], draw_xh, 0,
                    draw_main_y, blitters_10h_transp, 0);
      if (curr_modifier & 0x80) {  // blood
        add_backtable(id_chtab_6_environment, var_4 + 114, draw_xh + 1, 4,
                      draw_main_y - 6, blitters_4Ch_mono_12, 0);
      }
      add_backtable(id_chtab_6_environment, chomper_fram_top[var_4], draw_xh, 0,
                    draw_main_y - chomper_fram_y[var_4], blitters_10h_transp,
                    0);
      break;
  }
}
...
void __pascal far draw_tile_fore() {
  ...
  switch (curr_tile) {
    case tiles_2_spike:
      add_foretable(id_chtab_6_environment,
                    spikes_fram_fore[get_spike_frame(curr_modifier)], draw_xh,
                    0, draw_main_y - 2, blitters_10h_transp, 0);
      break;
    case tiles_18_chomper:
      var_2 = chomper_fram1[MIN(curr_modifier & 0x7F, 6)];
      add_foretable(id_chtab_6_environment, chomper_fram_for[var_2], draw_xh, 0,
                    draw_main_y, blitters_10h_transp, 0);
      if (curr_modifier & 0x80) {
        add_foretable(id_chtab_6_environment, var_2 + 119, draw_xh + 1, 4,
                      draw_main_y - 6, blitters_4Ch_mono_12, 0);
      }
      break;
    case tiles_20_wall:
      if (custom->tbl_level_type[current_level] == 0 ||
          custom->enable_wda_in_palace || graphics_mode != gmMcgaVga) {
        add_foretable(id_chtab_7_environmentwall,
                      wall_fram_main[curr_modifier & 0x7F], draw_xh, 0,
                      draw_main_y, blitters_0_no_transp, 0);
      }
    ...
    default:
      ...
      if (curr_tile == tiles_10_potion) {
        ...
        if (potion_type < 5 && potion_type >= 2)
      ...
      if (curr_tile == tiles_10_potion) {
        ...
        add_foretable(id_chtab_1_flameswordpotion, id, xh, 6, ybottom,
                      blitters_10h_transp, 0);
      } else {
        if ((curr_tile == tiles_3_pillar &&
             custom->tbl_level_type[current_level] == 0) ||
            (curr_tile >= tiles_27_lattice_small &&
             curr_tile < tiles_30_torch_with_debris)) {
          add_foretable(id_chtab_6_environment, id, xh, 0, ybottom,
                        blitters_0_no_transp, 0);
        } else {
          add_foretable(id_chtab_6_environment, id, xh, 0, ybottom,
                        blitters_10h_transp, 0);
...
// seg008:10A8
int __pascal far add_backtable(short chtab_id, int id, sbyte xh, sbyte xl,
...
// seg008:1017
int __pascal far add_foretable(short chtab_id, int id, sbyte xh, sbyte xl,
                               int ybottom, int blit, byte peel) {
  word index;
  if (id == 0) return 0;
...
// seg008:113A
int __pascal far add_midtable(short chtab_id, int id, sbyte xh, sbyte xl,
                              int ybottom, int blit, byte peel) {
  word index;
  if (id == 0) {
    return 0;
...
// seg008:1254
void __pascal far add_wipetable(sbyte layer, short left, short bottom,
                                sbyte height, short width, sbyte color) {
  word index;
...
void __pascal far draw_back_fore(int which_table, int index) {
  ...
  */
  draw_image(image, mask, table_entry->xh * 8 + table_entry->xl, table_entry->y,
             table_entry->blit);
}
...
SDL_Surface* hflip(SDL_Surface* input) {
  ...
  // Temporarily turn off alpha and colorkey on input. So we overwrite the
  // output image.
  SDL_SetColorKey(input, SDL_FALSE, 0);
  SDL_SetColorKey(output, SDL_FALSE, 0);
  SDL_SetSurfaceAlphaMod(input, 255);

  for (source_x = 0, target_x = width - 1; source_x < width;
       ++source_x, --target_x) {
...
void __pascal far draw_mid(int index) {
  ...
  /*
  if ((graphics_mode == gmCga || graphics_mode == gmHgaHerc) &&
  chtab_shift[chtab_id]) { mask = chtab_addrs[chtab_id]->images[image_id +
  chtab_addrs[chtab_id]->n_images / 2];
  }
  */
  ...
  if (midtable_entry->peel) {
    add_peel(round_xpos_to_byte(xpos, 0),
             round_xpos_to_byte(image->w /*width*/ + xpos, 1), ypos,
             image->h /*height*/);
  }
  // printf("Midtable: drawing (chtab %d, image %d) at (x=%d,
...
// seg008:167B
void __pascal far draw_image(image_type far* image, image_type far* mask,
                             int xpos, int ypos, int blit) {
  rect_type rect;
  switch (blit) {
...
void __pascal far draw_gate_back() {
  ...
  if (gate_bottom_y + 12 < draw_main_y) {
    add_backtable(id_chtab_6_environment, 50 /*gate bottom with B*/, draw_xh, 0,
                  gate_bottom_y, blitters_0_no_transp, 0);
  } else {
    // The following line (erroneously) erases the top-right of the tile
    // below-left (because it is drawn non-transparently).
    // -- But it draws something that was already drawn! (in draw_tile_right()).
    add_backtable(id_chtab_6_environment, tile_table[tiles_4_gate].right_id,
                  draw_xh, 0, tile_table[tiles_4_gate].right_y + draw_main_y,
                  blitters_0_no_transp, 0);
    ...
    draw_loose(0);
    draw_tile_base();
    add_backtable(id_chtab_6_environment, 51 /*gate bottom*/, draw_xh, 0,
                  gate_bottom_y - 2, blitters_10h_transp, 0);
  }
  ybottom = gate_bottom_y - 12;
  if (ybottom < 192) {
    for (; ybottom >= 0 && ybottom > 7 && ybottom - 7 > gate_top_y;
         ybottom -= 8) {
      add_backtable(id_chtab_6_environment, 52 /*gate slice 8px*/, draw_xh, 0,
                    ybottom, blitters_0_no_transp, 0);
    }
  }
  var_2 = ybottom - gate_top_y + 1;
  if (var_2 > 0 && var_2 < 9) {
    add_backtable(id_chtab_6_environment, door_fram_slice[var_2], draw_xh, 0,
                  ybottom, blitters_0_no_transp, 0);
  }
}
...
void __pascal far draw_gate_fore() {
  short ybottom;
  calc_gate_pos();
  add_foretable(id_chtab_6_environment, 51 /*gate bottom*/, draw_xh, 0,
                gate_bottom_y - 2, blitters_10h_transp, 0);
  ybottom = gate_bottom_y - 12;
  if (ybottom < 192) {
    for (; ybottom >= 0 && ybottom > 7 && ybottom - 7 > gate_top_y;
         ybottom -= 8) {
      add_foretable(id_chtab_6_environment, 52 /*gate slice 8px*/, draw_xh, 0,
                    ybottom, blitters_10h_transp, 0);
    }
  }
}
...
void __pascal far load_alter_mod(int tilepos) {
  ...
  switch (tiletype) {
    ...
    case tiles_20_wall: {
      ...
      //*curr_tile_modif <<= 7; // original: "no blue" mod becomes most
      //significant bit
      if (stored_modif == 1)
        *curr_tile_modif = 0x80;
      else
        *curr_tile_modif = (stored_modif << 4);
      // retain three bits more information:
      // most significant bit:       1 ==> "no blue"
      // next 3 bits:                for displaying various fake tiles
      // (invisible walls)
      // ..
...
// Here, ahead of time, we define the condition for a neighbouring tile to be
// seen as a wall connection
#define read_adj_tile_modif_in_curr_room()      // nop; access to modifier of
                                                // adjacent tiles is not needed
      ...
      if (graphics_mode != gmCga && graphics_mode != gmHgaHerc) {
        ...
        if (tilepos % 10 == 0) {
          if (room_L) {
            ...
            read_adj_tile_modif_in_external_room();  // only executed when fake
                                                     // tiles are enabled
...
void __pascal far add_drect(rect_type* source) {
  ...
  for (index = 0; index < drects_count; ++index) {
    if (intersect_rect(&target_rect, shrink2_rect(&target_rect, source, -1, -1),
                       &drects[index])) {
...
void __pascal far draw_leveldoor() {
  ...
  add_backtable(id_chtab_6_environment, 99 /*leveldoor stairs bottom*/,
                draw_xh + 1, 0, ybottom, blitters_0_no_transp, 0);
  if (modifier_left) {
    if (level.start_room != drawn_room) {
      add_backtable(id_chtab_6_environment, 144 /*level door stairs*/,
                    draw_xh + 1, 0, ybottom - 4, blitters_0_no_transp, 0);
    } else {
      short leveldoor_width =
          (custom->tbl_level_type[current_level] == 0) ? 39 : 48;
      sbyte x_low =
          (custom->tbl_level_type[current_level] == 0)
              ? 2
              : 0;  // dungeon level doors are shifted 2px to the right
      add_wipetable(0, 8 * (draw_xh + 1) + x_low, ybottom - 4, 45,
                    leveldoor_width, 0);
    }
  }
  leveldoor_ybottom = ybottom - (modifier_left & 3) - 48;
  for (var_6 = ybottom - modifier_left;
       add_backtable(id_chtab_6_environment, 33 /*level door bottom*/,
                     draw_xh + 1, 0, leveldoor_ybottom, blitters_0_no_transp,
                     0),
      var_6 > leveldoor_ybottom;
       leveldoor_ybottom += 4) {
    ;
  }  // runs at least once?
  add_backtable(id_chtab_6_environment, 34 /*level door top*/, draw_xh + 1, 0,
                draw_main_y - 64, blitters_0_no_transp, 0);
}
...
void __pascal far get_room_address(int room) {
  if (room < 0 || room > 24) {
    // printf("Tried to access room %d, not in 0..24.\n", room);
    interesting = 1;
  }
...
void __pascal far draw_floor_overlay() {
  ...
  if (curr_tile == tiles_1_floor || curr_tile == tiles_3_pillar ||
      curr_tile == tiles_5_stuck || curr_tile == tiles_19_torch) {
    // frames 137..144: climb
    // index overflow here?
    if (Kid.frame >= frame_137_climbing_3 &&
        Kid.frame <= frame_144_climbing_10) {
      add_midtable(id_chtab_6_environment, floor_left_overlay[Kid.frame - 137],
                   draw_xh, 0, (curr_tile == tiles_5_stuck) + draw_main_y,
                   blitters_10h_transp, 0);
    } else {
      // triggered by 02-random-broken
      printf(
          "draw_floor_overlay: attempted to draw floor overlay with frame %d "
          "not in 137..144\n",
          Kid.frame);
      // quit(1);
    }
...
void __pascal far draw_other_overlay() {
  ...
  if (tile_left == tiles_0_empty) {
    ...
    draw_tile2();
             ...
             get_tile_to_draw(drawn_room, drawn_col - 2, drawn_row, &tiletype,
                              &modifier, tiles_0_empty) == tiles_0_empty) {
    ptr_add_table = &add_midtable;
    draw_tile2();
    ptr_add_table = &add_backtable;
    draw_tile2();
...
void __pascal far draw_objtable_items_at_tile(byte tilepos) {
  ...
  if (obj_count) {
    for (obj_index = obj_count - 1, n_curr_objs = 0; obj_index >= 0;
         --obj_index) {
...
void __pascal far draw_objtable_item(int index) {
  switch (load_obj_from_objtable(index)) {
    ...
    shadow:
      ...
      add_midtable(obj_chtab, obj_id + 1, obj_xh, obj_xl, obj_y, blitters_2_or,
                   1);
    ...
    case 0x80:  // loose floor
      ...
      add_midtable(obj_chtab, loose_fram_left[obj_id], obj_xh, obj_xl,
                   obj_y - 3, blitters_10h_transp, 1);
      add_midtable(obj_chtab, loose_fram_bottom[obj_id], obj_xh, obj_xl, obj_y,
                   0, 1);
      add_midtable(obj_chtab, loose_fram_right[obj_id], obj_x + 4, obj_xl,
                   obj_y - 1, blitters_10h_transp, 1);
      break;
  }
}
...
void __pascal far add_guard_to_objtable() {
  ...
  if (Char.charid == charid_1_shadow) {
    ...
    if (current_level == /*4*/ custom->mirror_level &&
        Char.room == /*4*/ custom->mirror_room) {
      obj_clip_left = 137;
    ...
    if (rem_min > 0) {
      if (rem_min == 1) {
        ...
        if (rem_sec == 1) {
          ...
          snprintf(sprintf_temp, sizeof(sprintf_temp), "%d SECONDS LEFT",
                   rem_sec);
        }
      } else {
        snprintf(sprintf_temp, sizeof(sprintf_temp), "%d MINUTES LEFT",
                 rem_min);
      }
...
void __pascal far show_level() {
  ...
  if (disp_level != 0 &&
      disp_level < /*14*/ custom->hide_level_number_from_level &&
      seamless == 0) {
    if (disp_level == 13) {
...
void __pascal far wall_pattern(int which_part, int which_table) {
  ...
  is_dungeon = (custom->tbl_level_type[current_level] < DESIGN_PALACE) ||
               custom->enable_wda_in_palace;
  if ((!is_dungeon) && (graphics_mode == GRAPHICS_VGA)) {
    // I haven't traced the palace WDA
    //[...]
    if (which_part) {
      add_wipetable(which_table, 8 * (draw_xh), draw_main_y - 40, 20, 4 * 8,
                    palace_wall_colors[44 * drawn_row + drawn_col]);
      add_wipetable(which_table, 8 * (draw_xh), draw_main_y - 19, 21, 2 * 8,
                    palace_wall_colors[44 * drawn_row + 11 + drawn_col]);
      add_wipetable(which_table, 8 * (draw_xh + 2), draw_main_y - 19, 21, 2 * 8,
                    palace_wall_colors[44 * drawn_row + 12 + drawn_col]);
      add_wipetable(which_table, 8 * (draw_xh), draw_main_y, 19, 1 * 8,
                    palace_wall_colors[44 * drawn_row + 22 + drawn_col]);
      add_wipetable(which_table, 8 * (draw_xh + 1), draw_main_y, 19, 3 * 8,
                    palace_wall_colors[44 * drawn_row + 23 + drawn_col]);
      ptr_add_table(id_chtab_7_environmentwall, prandom(2) + 3, draw_xh + 3, 0,
                    draw_main_y - 53, blitters_46h_mono_6, 0);
      ptr_add_table(id_chtab_7_environmentwall, prandom(2) + 6, draw_xh, 0,
                    draw_main_y - 34, blitters_46h_mono_6, 0);
      ptr_add_table(id_chtab_7_environmentwall, prandom(2) + 9, draw_xh, 0,
                    draw_main_y - 13, blitters_46h_mono_6, 0);
      ptr_add_table(id_chtab_7_environmentwall, prandom(2) + 12, draw_xh, 0,
                    draw_main_y, blitters_46h_mono_6, 0);
    }
    add_wipetable(which_table, 8 * draw_xh, draw_bottom_y, 3, 4 * 8,
                  palace_wall_colors[44 * drawn_row + 33 + drawn_col]);
    ptr_add_table(id_chtab_7_environmentwall, prandom(2) + 15, draw_xh, 0,
                  draw_bottom_y, blitters_46h_mono_6, 0);
  } else {
    v3 = prandom(1);
    v5 = prandom(4);
    v2 = prandom(1);
    ...
    // apparently, for walls, the modifier stores whether there are adjacent
    // walls
    ...
    switch (bg_modifier) {
      case WALL_MODIFIER_WWW:
        if (which_part != 0) {
          if (prandom(4) == 0) {
            ptr_add_table(RSET_WALL, RES_WALL_RNDBLOCK, draw_xh, 0,
                          draw_bottom_y - 42, BLIT_NO_TRANS, 0);
          }
          ptr_add_table(RSET_WALL, RES_WALL_DIVIDER1 + v3, draw_xh + 1, v5,
                        draw_bottom_y - 21, BLIT_TRANS, 0);
        }
        ptr_add_table(RSET_WALL, RES_WALL_DIVIDER1 + v2, draw_xh, v4,
                      draw_bottom_y, BLIT_TRANS, 0);
        if (which_part != 0) {
          if (is_dungeon) {
            if (prandom(4) == 0) {
      ...
      case WALL_MODIFIER_SWW:
        if (which_part != 0) {
          ...
          ptr_add_table(RSET_WALL, RES_WALL_DIVIDER1 + v3, draw_xh + 1 /*fix*/,
                        v5, draw_bottom_y - 21, BLIT_TRANS, 0);
          if (is_dungeon) {
            if (prandom(4) == 0) {
              draw_right_mark(prandom(3), v5);
            }
...
void __pascal far draw_left_mark(word arg3, word arg2, word arg1) {
  ...
  ptr_add_table(RSET_WALL, lv1, draw_xh + (arg3 == 2 || arg3 == 3), lv2,
                draw_bottom_y - LPOS[arg3], BLIT_TRANS, 0);
}
...
void __pascal far draw_right_mark(word arg2, word arg1) {
  ...
  ptr_add_table(RSET_WALL, rv, draw_xh + (arg2 > 1), arg1,
                draw_bottom_y - RPOS[arg2], BLIT_TRANS, 0);
}
