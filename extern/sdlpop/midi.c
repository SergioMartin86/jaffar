...
CREDITS:
    The OPL integration code is based in part on Chocolate Doom's MIDI
interface, by Simon Howard (GPLv2-licensed). Uses the Nuked OPL3 emulator by
Alexey Khokholov (GPLv2-licensed). MIDI playback code also published as
standalone playback program 'popmidi' by Falcury (GPLv3)
*/
...
// OPL3 supports more channels (voices) than OPL2: you could increase the number
// of used voices (try it!) However, released notes can linger longer, so for
// some tracks it will sound too 'washed out'. For example, listen to these
// sounds with 18 voices enabled:
...
static float
    midi_current_pos_fract_part;  // partial ticks after the decimal point
...
//   Speed must be exactly right, otherwise it will not line up with the
//   flashing animation in the cutscene. The 'Jaffar enters' song has tempo
//   705882 (maybe this tempo was carefully fine-tuned)?
...
static instrument_type hardcoded_instrument = {
    0x13, 0x09,
    0x04, {{0x02, 0x8D, 0xD7, 0x37, 0x00}, {0x03, 0x03, 0xF5, 0x18, 0x00}},
    0x00, {0x00, 0x00},
};
...
static dword midi_read_variable_length(byte** buffer_position) {
  ...
  for (i = 0; i < 4; ++i) {
    ...
    if ((pos[i] & 0x80) == 0)
      break;  // The most significant bit being 0 means that this is the last
              // u8.
  }
  *buffer_position +=
      i + 1;  // Advance the pointer, so we know where the next field starts.
  return result;
}
...
bool parse_midi(midi_raw_chunk_type* midi, parsed_midi_type* parsed_midi) {
  ...
  if (memcmp(midi->chunk_type, "MThd", 4) != 0) {
    printf(
        "Warning: Tried to play a midi sound without the 'MThd' chunk "
        "header.\n");
    return 0;
  }
  ...
  if (midi_format >= 2) {
    printf(
        "Warning: Unsupported midi format %d (only type 0 or 1 files are "
        "supported)\n",
        midi_format);
    return 0;
  }
  ...
  if (division < 0) {
    division =
        (-(division / 256)) *
        (division &
         0xFF);  // Translate time delta from the alternative SMTPE format.
  }
  ...
  midi_raw_chunk_type* next_track_chunk =
      (midi_raw_chunk_type*)midi->header
          .tracks;  // The first track chunk starts after the header chunk.
  ...
  for (int track_index = 0; track_index < num_tracks; ++track_index) {
    ...
    next_track_chunk =
        (midi_raw_chunk_type*)(track_chunk->data +
                               (dword)SDL_SwapBE32(track_chunk->chunk_length));
    midi_track_type* track = &parsed_midi->tracks[track_index];
    byte* buffer_position = track_chunk->data;
    for (;;) {
      track->events = realloc(track->events,
                              (++track->num_events) * sizeof(midi_event_type));
      ...
      if (event->event_type & 0x80) {
        ...
        event->event_type =
            last_event_type;  // Implicit use of the previous event type.
      }
      ...
      switch (event->event_type & 0xF0) {
        ...
        default:
          ...
          switch (event->event_type) {
            ...
            default:
              printf(
                  "Warning: unknown midi event type 0x%02x (track %d, event "
                  "%d)\n",
      ...
      if (event->event_type == 0xFF /* meta event */ &&
          event->meta.type == 0x2F /* end of track */) {
        break;
      }
...
static word reg_pair_offsets[] = {
    0x000, 0x001, 0x002, 0x003, 0x004, 0x005, 0x008, 0x009, 0x00A,
    0x00B, 0x00C, 0x00D, 0x010, 0x011, 0x012, 0x013, 0x014, 0x015,
    0x100, 0x101, 0x102, 0x103, 0x104, 0x105, 0x108, 0x109, 0x10A,
    0x10B, 0x10C, 0x10D, 0x110, 0x111, 0x112, 0x113, 0x114, 0x115};

static word reg_single_offsets[] = {0,     1,     2,     3,     4,     5,
                                    6,     7,     8,     0x100, 0x101, 0x102,
                                    0x103, 0x104, 0x105, 0x106, 0x107, 0x108};
...
static void opl_write_instrument(instrument_type* instrument, byte voice) {
  opl_write_reg(0xC0 + reg_single_offsets[voice],
                instrument->FB_conn | 0x30 /* OPL3: L+R speaker enable */);
...
static void midi_note_off(midi_event_type* event) {
  ...
  for (int voice = 0; voice < NUM_OPL_VOICES; ++voice) {
    if (voice_channel[voice] == channel && voice_note[voice] == note) {
      opl_write_reg_masked(0xB0 + reg_single_offsets[voice], 0,
                           0x20);  // release key
...
static void midi_note_on(midi_event_type* event) {
  ...
  if (velocity == 0) {
    ...
    for (int i = 0; i < NUM_OPL_VOICES; ++i) {
      // Don't use the same voice immediately again: that note is probably still
      // be in the release phase.
      ++test_voice;
    ...
    if (voice >= 0) {
      ...
      // However, Prince of Persia treats notes as one octave (12 semitones)
      // lower than that, by default. A special MIDI SysEx event is used to
      // change the frequency of all notes.
      float octaves_from_A4 =
          ((int)event->channel.param1 - 69 - 12 + midi_semitones_higher) /
          12.0f;
      float frequency = powf(2.0f, octaves_from_A4) * 440.0f;
      float f_number_float = frequency * (float)(1 << 20) / 49716.0f;
      ...
      //			opl_write_reg_masked(0xB0 + reg_offset, 0,
      //0x20); // Turn note off first (should not be necessary)
      ...
      opl_write_reg_masked(0x40 + opl_reg_pair_offset(voice, 0),
                           instrument->operators[0].ksl_tl, 0x3F);
      ...
      carrier_volume -= 64;
      opl_write_reg_masked(0x40 + opl_reg_pair_offset(voice, 1), carrier_volume,
                           0x3F);
    } else {
    ...
    case 0xF0:  // SysEx event:
      if (event->sysex.length == 7) {
        ...
        if (data[2] == 0x34 && (data[3] == 0 || data[3] == 1) && data[4] == 0) {
          midi_semitones_higher =
              data[5];  // Make all notes higher by this amount.
        }
      }
    case 0xFF:  // Meta event
      switch (event->meta.type) {
        ...
        case 0x51:  // set tempo
        {
          ...
          int new_tempo = (data[0] << 16) | (data[1] << 8) | (data[2]);
          new_tempo *=
              (1.0f + current_midi_tempo_modifier);  // tempo adjustment for
                                                     // specific songs
...
void midi_callback(void* userdata, Uint8* stream, int len) {
  ...
  while (frames_needed > 0) {
    if (ticks_to_next_pause > 0) {
      // Fill the audio buffer (we have already processed the MIDI events up
      // till this point)
      int64_t us_to_next_pause =
          ticks_to_next_pause * us_per_beat / ticks_per_beat;
      int64_t us_needed = frames_needed * ONE_SECOND_IN_US / mixing_freq;
      int64_t advance_us = MIN(us_to_next_pause, us_needed);
      int available_frames =
          ((advance_us * mixing_freq) + ONE_SECOND_IN_US - 1) /
          ONE_SECOND_IN_US;  // round up.
      int advance_frames = MIN(available_frames, frames_needed);
      advance_us =
          advance_frames * ONE_SECOND_IN_US /
          mixing_freq;  // recalculate, in case the rounding up increased this.
      ...
      // Keep track of the partial ticks that have elapsed so that we do not
      // fall behind.
      float ticks_elapsed_float =
          (float)advance_us * ticks_per_beat / us_per_beat;
      ...
      for (int track_index = 0; track_index < num_midi_tracks; ++track_index) {
        ...
        while (midi_current_pos >= track->next_pause_tick) {
          ...
          if (events_left > 0) {
            ...
            //						print_midi_event(track_index, track->event_index-1,
            //event);
            process_midi_event(event);

            // Need to look ahead: must delay processing of the next event, if
            // there is a pause.
            if (events_left > 1) {
      ...
      if (num_finished_tracks >= num_midi_tracks) {
        // All tracks have finished. Fill the remaining samples with silence and
        // stop playback.
        SDL_memset(stream, 0, frames_needed * 4);
        //				printf("midi_callback(): sound
        //ended\n");
        SDL_LockAudio();
        midi_playing = 0;
        free_parsed_midi(&parsed_midi);
        ...
        for (int i = 0; i < num_midi_tracks; ++i) {
          ...
          if (track->event_index >= track->num_events ||
              midi_current_pos >= track->next_pause_tick)
            continue;
          first_next_pause_tick =
              MIN(first_next_pause_tick, track->next_pause_tick);
        }
        if (first_next_pause_tick == INT64_MAX) {
          printf(
              "MIDI: Couldn't figure out how long to delay (this is a bug)\n");
          quit(1);
        }
        ticks_to_next_pause = first_next_pause_tick - midi_current_pos;
        if (ticks_to_next_pause < 0) {
          printf(
              "Tried to delay a negative amount of time (this is a bug)\n");  // This should never happen?
          quit(1);
        }
        //				printf("                             delaying
        //%d ticks = %.3f s\n", 				       ticks_to_next_pause, (((us_per_beat /
        //ticks_per_beat) * (dword)ticks_to_next_pause) / 1e6f));
      }
    }
  }
}
...
void free_midi_resources() { free(instruments_data); }
...
void init_midi() {
  ...
  instruments =
      &hardcoded_instrument;  // unused if instruments can be loaded normally.
  int size;
...
void play_midi_sound(sound_buffer_type far* buffer) {
  ...
  opl_write_reg(0x105, 0x01);  // OPL3 enable (note: the PoP1 Adlib sounds don't
                               // actually use OPL3 extensions)
  for (int voice = 0; voice < NUM_OPL_VOICES; ++voice) {
