...
void restore_room_after_quick_load() {
  // word old_exit_room_timer = exit_room_timer;
  ...
  // exit_room_timer = old_exit_room_timer;
}
  ...
  if (start_level < 0) {
    if (key || control_shift) {
      ...
      if (key == SDL_SCANCODE_TAB || need_start_replay) {
        ...
        start_recording();
      } else if (key == (SDL_SCANCODE_L | WITH_CTRL)) {  // ctrl-L
  ...
  if (cheats_enabled) {
    switch (key) {
      ...
      case SDL_SCANCODE_N:  // N --> view room below
        ...
        next_room = room_B;
        break;
      case SDL_SCANCODE_B | WITH_CTRL:  // Ctrl+B --> Go back to the room where
                                        // the prince is. (Undo H,J,U,N.)
        draw_guard_hp(0, 10);
        next_room = Kid.room;
        break;
...
void __pascal far load_lev_spr(int level) {
  ...
  if (guardtype != -1) {
    if (guardtype == 0) {
      dathandle = open_dat(
          custom->tbl_level_type[current_level] ? "GUARD1.DAT" : "GUARD2.DAT",
          0);
    }
...
#define DEGREES_TO_RADIANS (M_PI / 180.0)
  ...
  if ((dword)dist_squared < (dword)(joystick_threshold * joystick_threshold)) {
    ...
    // necessary)
             ...
             angle <
                 (145 * DEGREES_TO_RADIANS))  // 110 degree range facing down
      axis_state[1] = 1;
...
void __pascal far read_joyst_control() {
  if (joystick_only_horizontal) {
    get_joystick_state_hor_only(joy_axis[SDL_CONTROLLER_AXIS_LEFTX],
                                joy_left_stick_states);
    get_joystick_state_hor_only(joy_axis[SDL_CONTROLLER_AXIS_RIGHTX],
                                joy_right_stick_states);
  } else {
    get_joystick_state(joy_axis[SDL_CONTROLLER_AXIS_LEFTX],
                       joy_axis[SDL_CONTROLLER_AXIS_LEFTY],
                       joy_left_stick_states);
    get_joystick_state(joy_axis[SDL_CONTROLLER_AXIS_RIGHTX],
                       joy_axis[SDL_CONTROLLER_AXIS_RIGHTY],
                       joy_right_stick_states);
  }
  ...
  if (joy_left_stick_states[1] == -1 || joy_right_stick_states[1] == -1 ||
      joy_hat_states[1] == -1 || joy_AY_buttons_state == -1)
    control_y = -1;
...
// seg000:1200
void __pascal far set_health_life() { hitp_delta = hitp_max - hitp_curr; }
...
// seg009:12EF
void __pascal far load_one_optgraf(chtab_type* chtab_ptr,
                                   dat_pal_type far* pal_ptr, int base_id,
...
void __pascal far read_keyb_control() {
  if (key_states[SDL_SCANCODE_UP] || key_states[SDL_SCANCODE_HOME] ||
      key_states[SDL_SCANCODE_PAGEUP] || key_states[SDL_SCANCODE_KP_8] ||
      key_states[SDL_SCANCODE_KP_7] || key_states[SDL_SCANCODE_KP_9]) {
    control_y = -1;
  ...
  if (key_states[SDL_SCANCODE_LEFT] || key_states[SDL_SCANCODE_HOME] ||
    ...
    control_x = -1;
             ...
             key_states[SDL_SCANCODE_PAGEUP] || key_states[SDL_SCANCODE_KP_6] ||
             key_states[SDL_SCANCODE_KP_9]) {
    control_x = 1;
  }
  control_shift =
...
void show_splash() {
  ...
  current_target_surface = onscreen_surface_;
  draw_rect(&screen_rect, 0);
  show_text_with_color(&splash_text_1_rect, 0, 0, splash_text_1,
                       color_15_brightwhite);
  show_text_with_color(&splash_text_2_rect, 0, -1, splash_text_2,
                       color_7_lightgray);
  ...
  do {
    ...
    if (joy_hat_states[0] != 0 || joy_X_button_state != 0 ||
        joy_AY_buttons_state != 0 || joy_B_button_state != 0) {
      joy_hat_states[0] = 0;
      joy_AY_buttons_state = 0;
      joy_X_button_state = 0;
      joy_B_button_state = 0;
      key_states[SDL_SCANCODE_LSHIFT] =
          1;  // close the splash screen using the gamepad
    }
    delay_ticks(1);

  } while (key == 0 && !(key_states[SDL_SCANCODE_LSHIFT] ||
  ...
  key_states[SDL_SCANCODE_LSHIFT] =
      0;  // don't immediately start the game if shift was pressed!
  key_states[SDL_SCANCODE_RSHIFT] = 0;
}
