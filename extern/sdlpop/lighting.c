...
void init_lighting() {
  ...
  screen_overlay = SDL_CreateRGBSurface(0, 320, 192, 32, 0xFF << 0, 0xFF << 8,
                                        0xFF << 16, 0xFF << 24);
  ...
  // ambient lighting
  bgcolor = SDL_MapRGBA(screen_overlay->format, ambient_level, ambient_level,
                        ambient_level, SDL_ALPHA_OPAQUE);
}
...
void redraw_lighting() {
  ...
  for (int tile_pos = 0; tile_pos < 30; tile_pos++) {
    ...
    if (tile_type == tiles_19_torch ||
        tile_type == tiles_30_torch_with_debris) {
      // Center of the flame.
      ...
      int result =
          SDL_BlitSurface(lighting_mask, NULL, screen_overlay, &dest_rect);
      if (result != 0) {
...
void update_lighting(const rect_type far* target_rect_ptr) {
  ...
  int result =
      SDL_BlitSurface(screen_overlay, &sdlrect, onscreen_surface_, &sdlrect);
  if (result != 0) {
