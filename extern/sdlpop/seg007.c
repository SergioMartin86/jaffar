...
void __pascal far animate_torch() {
  ...
  // Keep animating torches in the rightmost column of the left-side room as
  // well, because they are visible in the current room.
  if (trob.room == drawn_room ||
      (trob.room == room_L && (trob.tilepos % 10) == 9)) {
          ...
          if (curr_modifier != 0) {
            if (curr_modifier < 188) {
              if ((curr_modifier & 3) == 3) {
                play_door_sound_if_visible(
                    sound_4_gate_closing);  // gate closing
              }
            }
          } else {
            gate_stop();
  ...
  if (trob.type >= 0) {
    if (trob_type >= 3) {
      ...
      if ((sbyte)curr_modifier < 0) {
        ...
        if (trob.type == 4 && (sound_flags & sfDigi)) {
          sound_interruptible[sound_15_leveldoor_sliding] = 1;
          play_sound(
              sound_15_leveldoor_sliding);  // level door sliding (closing)
        }
      }
    } else {
      // opening
      ...
      if (curr_modifier >= 43) {
        ...
        if (leveldoor_open == 0 || leveldoor_open == 2) {
          ...
          if (current_level == /*4*/ custom->mirror_level) {
            ...
            get_tile(/*4*/ custom->mirror_room, /*4*/ custom->mirror_column,
                     /*0*/ custom->mirror_row);
            curr_room_tiles[curr_tilepos] =
                /*tiles_13_mirror*/ custom->mirror_tile;
          }
        }
      } else {
...
void __pascal far set_redraw2(short tilepos, byte frames) {
  if (tilepos < 30) {
    if (tilepos < 0) {
      ...
      // however, loose tiles falling out of the room end up with a negative
      // tilepos {-2 .. -11} !
      tilepos = (-tilepos) - 1;
...
short __pascal far trigger_1(short target_type, short room, short tilepos,
  ...
  } else if (custom->allow_triggering_any_tile) {  // allow_triggering_any_tile
                                                   // hack
    result = 1;
  }
  return result;
...
void __pascal far do_trigger_list(short index, short button_type) {
  ...
  while (1) {  // Same as the above but just a little faster and no compiler
               // warning.
...
short __pascal far find_trob() {
  short index;
  for (index = 0; index < trobs_count; ++index) {
    if (trobs[index].tilepos == trob.tilepos && trobs[index].room == trob.room)
      return index;
  }
  return -1;
}
...
void __pascal far clear_tile_wipes() {
  ...
  memset_near(redraw_frames_floor_overlay, 0,
              sizeof(redraw_frames_floor_overlay));
...
void __pascal far animate_button() {
  word var_2;
  if (trob.type >= 0) {
    set_doorlink_timer(curr_modifier,
                       var_2 = get_doorlink_timer(curr_modifier) - 1);
    if (var_2 < 2) {
      trob.type = -1;
      redraw_11h();
    }
...
int __pascal far remove_loose(int room, int tilepos) {
  ...
  // note: the level type is used to determine the modifier of the empty space
  // left behind
...
void __pascal far start_chompers() {
  ...
  if ((byte)Char.curr_row < 3) {
    ...
    for (column = 0, tilepos = tbl_line[Char.curr_row]; column < 10;
         ++column, ++tilepos) {
      if (get_curr_tile(tilepos) == tiles_18_chomper) {
        modifier = curr_modifier & 0x7F;
        if (modifier == 0 || modifier >= 6) {
          start_anim_chomper(Char.room, tilepos,
                             timing | (curr_modifier & 0x80));
...
void __pascal far move_loose() {
  ...
  if (curmob.y < 226 && y_something[curmob.row + 1] <= curmob.y) {
    ...
    if (curr_tile_temp == tiles_0_empty || curr_tile_temp == tiles_11_loose) {
      mob_down_a_row();
      return;
    }
...
void __pascal far sub_9A8E() {
  ...
  method_1_blit_rect(onscreen_surface_, offscreen_surface, &rect_top, &rect_top,
                     0);
}
...
void __pascal far check_loose_fall_on_kid() {
  loadkid();
  if (Char.room == curmob.room && Char.curr_col == curmob.xh >> 2 &&
      curmob.y < Char.y && Char.y - 30 < curmob.y) {
    fell_on_your_head();
    savekid();
  }
}
...
void __pascal far fell_on_your_head() {
  ...
  if ((current_level == 13 || (frame < frame_5_start_run || frame >= 15)) &&
    ...
    if (take_hp(1)) {
      ...
      if (frame != frame_109_crouch) {  // crouching
        ...
        seqtbl_offset_char(
            seq_52_loose_floor_fell_on_kid);  // loose floor fell on Kid
      }
    }
  }
}
