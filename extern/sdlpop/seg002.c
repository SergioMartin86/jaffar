...
void __pascal far get_guard_hp() {
  guardhp_delta = guardhp_curr = guardhp_max =
      custom->extrastrength[guard_skill] + custom->tbl_guard_hp[current_level];
}
...
void __pascal far enter_guard() {
  ...
  if (graphics_mode == gmMcgaVga &&
      custom->tbl_guard_type[current_level] == 0) {
  ...
  curr_guard_color &=
      0x0F;  // added; only least significant 4 bits are used for guard color
  ...
  if (seq_hi == 0) {
    if (Char.charid == charid_4_skeleton) {
      ...
      seqtbl_offset_char(
          seq_63_guard_stand_active);  // stand active (when entering room)
                                       // (skeleton)
    } else {
      Char.sword = sword_0_sheathed;
      seqtbl_offset_char(
          seq_77_guard_stand_inactive);  // stand inactive (when entering room)
    }
  } else {
  ...
  frame = Char.frame;
  if (frame == frame_185_dead || frame == frame_177_spiked ||
      frame == frame_178_chomped) {
    Char.alive = 1;
  ...
  if (Guard.charid == charid_1_shadow) {
    ...
    saveshad();
             ...
             (Guard.room = level.roomlinks[Guard.room - 1].down) ==
                 /*3*/ custom->skeleton_reappear_room) {
...
void __pascal far leave_guard() {
  word room_minus_1;
  if (Guard.direction == dir_56_none || Guard.charid == charid_1_shadow ||
      Guard.charid == charid_24_mouse) {
    return;
  }
  // arrays are indexed 0..23 instead of 1..24
  ...
  level.guards_color[room_minus_1] =
      curr_guard_color & 0x0F;  // restriction to 4 bits added
...
void __pascal far exit_room() {
  ...
  if (Guard.alive < 0 && Guard.sword == sword_2_drawn) {
    ...
    // kid_room_m1 might be 65535 (-1) when the prince fell out of the level (to
    // room 0) while a guard was active. In this case, the indexing in the
    // following condition crashes on Linux.
    if ((kid_room_m1 >= 0 && kid_room_m1 <= 23) &&
        (level.guards_tile[kid_room_m1] >= 30 ||
...
short __pascal far leave_room() {
  ...
  action = Char.action;
  frame = Char.frame;
  if (action != actions_5_bumped && action != actions_4_in_freefall &&
      action != actions_3_in_midair && (sbyte)chary < 10 &&
      (sbyte)chary > -16) {
    leave_dir = 2;  // up
  } else if (chary >= 211) {
    leave_dir = 3;  // down
    ...
    if (curr_tile2 != tiles_7_doortop_with_floor &&
        curr_tile2 != tiles_12_doortop && char_x_right >= 201) {
      leave_dir = 1;  // right
  ...
  switch (leave_dir) {
    ...
    case 3:  // down
      ...
      if (current_level == custom->falling_exit_level /*6*/ &&
          Char.room == custom->falling_exit_room /*1*/) {
        return -2;
      }
      break;
  }
...
void __pascal far level3_set_chkp() {
  ...
  if (current_level == /*3*/ custom->checkpoint_level &&
      Char.room == 7 /* TODO: add a custom option */) {
    checkpoint = 1;
...
void __pascal far sword_disappears() {
  ...
  if (current_level == 12 && Char.room == 18) {
    ...
    curr_room_modif[curr_tilepos] =
        0;  // added, a nonzero modifier may show fake tiles
  }
}
...
// seg002:0770
void __pascal far move_7() { control_shift = 0; }
...
// seg002:085A
void __pascal far autocontrol_Jaffar() { autocontrol_guard(); }

// seg002:085F
void __pascal far autocontrol_kid() { autocontrol_guard(); }
...
void __pascal far autocontrol_guard_active() {
  ...
  if (char_frame != frame_166_stand_inactive && char_frame >= 150 &&
      can_guard_see_kid != 1) {
    if (can_guard_see_kid == 0) {
      ...
      if (distance >= 12 &&
          ...
          opp_frame >= frame_102_start_fall_1 &&
          opp_frame < frame_118_stand_up_from_crouch_9 &&
...
void __pascal far guard_follows_kid_down() {
  ...
  if (opp_action == actions_2_hang_climb ||
      opp_action == actions_6_hang_straight) {
    return;
  }
...
void __pascal far guard_block() {
  word opp_frame;
  opp_frame = Opp.frame;
  if (opp_frame == frame_152_strike_2 || opp_frame == frame_153_strike_3 ||
...
void __pascal far hurt_by_sword() {
  ...
  loc_4276:
    if (get_tile_behind_char() != 0 ||
      ...
      inc_curr_row();
      seqtbl_offset_char(
          seq_81_kid_pushed_off_ledge);  // Kid/Guard is killed and pushed off
                                         // the ledge
    }
  } else {
  ...
  play_sound(Char.charid == charid_0_kid ? sound_13_kid_hurt
                                         : sound_12_guard_hurt);
  play_seq();
}
...
void __pascal far check_skel() {
  ...
  if (current_level == /*3*/ custom->skeleton_level &&
    ...
    get_tile(drawn_room, /*5*/ custom->skeleton_column,
             /*1*/ custom->skeleton_row);
