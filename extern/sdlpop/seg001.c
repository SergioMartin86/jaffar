...
rect_type hof_rects[MAX_HOF_COUNT] = {
    {84, 72, 96, 248},   {98, 72, 110, 248},  {112, 72, 124, 248},
    {126, 72, 138, 248}, {140, 72, 152, 248}, {154, 72, 166, 248},
};
...
void __pascal far draw_princess_room_bg() {
  ...
  redraw_needed_tiles();
  add_foretable(id_chtab_8_princessroom, 2 /*pillar piece*/, 30, 0, 167,
...
void __pascal far princess_crouching() {
  ...
  Char.y = 169;
  seqtbl_offset_char(
      seq_110_princess_crouching_PV2);  // princess crouching [PV2]
  play_seq();
}
...
void __pascal far cutscene_8() {
  ...
  seqtbl_offset_kid_char(
      seq_107_mouse_stand_up_and_go);  // mouse stand up and go
  if (proc_cutscene_frame(20)) return;
  seqtbl_offset_shad_char(
      seq_111_princess_stand_up_PV2);  // princess stand up [PV2]
  if (proc_cutscene_frame(20)) return;
  Kid.frame = 0;
  fade_out_1();
}
...
void __pascal far cutscene_9() {
  ...
  seqtbl_offset_shad_char(
      seq_112_princess_crouch_down_PV2);  // princess crouch down [PV2]
...
void __pascal far end_sequence_anim() {
  ...
  seqtbl_offset_shad_char(
      seq_108_princess_turn_and_hug);  // princess turn and hug [PV2]
  if (proc_cutscene_frame(5)) return;
...
void __pascal far princess_room_torch() {
  ...
  for (which = 2; which--;) {
    ...
    princess_torch_frame[which_torch] =
        get_torch_frame(princess_torch_frame[which_torch]);
    add_backtable(id_chtab_1_flameswordpotion,
                  princess_torch_frame[which_torch] + 1,
                  princess_torch_pos_xh[which_torch],
                  princess_torch_pos_xl[which_torch], 116, 0, 0);
  }
}
...
void __pascal far draw_hourglass() {
  if (hourglass_sandflow >= 0) {
    ...
    add_foretable(id_chtab_8_princessroom, hourglass_sandflow + 10, 20, 0, 164,
                  blitters_10h_transp, 0);
  }
  if (hourglass_state) {
    add_midtable(id_chtab_8_princessroom, hourglass_state + 2, 19, 0, 168,
...
void __pascal far end_sequence() {
  ...
  is_ending_sequence =
      true;  // added (fix being able to pause the game during the end sequence)
  load_opt_sounds(sound_56_ending_music,
                  sound_56_ending_music);  // winning theme
  play_sound_from_buffer(
      sound_pointers[sound_56_ending_music]);  // winning theme
  ...
  if (hof_index < MAX_HOF_COUNT && hof_index <= hof_count) {
    ...
    while (input_str(&rect, hof[hof_index].name, 24, "", 0, 4, color,
                     bgcolor) <= 0)
      ;
    restore_peel(peel);
...
// seg001:0CCD
void __pascal far load_intro(int which_imgs, cutscene_ptr_type func,
                             int free_sounds) {
  short current_star;
  draw_rect(&screen_rect, 0);
  if (free_sounds) {
  ...
  method_6_blit_img_to_scr(get_image(id_chtab_9_princessbed, 0), 0, 142,
                           blitters_2_or);
  ...
  if (NULL != chtab_addrs[id_chtab_8_princessroom])
...
star_type stars[N_STARS] = {
    {20, 97, 0},  {16, 104, 1}, {23, 110, 2},
    {17, 116, 3}, {24, 120, 4}, {18, 128, 0},
};
...
void __pascal far show_hof() {
  ...
  for (index = 0; index < hof_count; ++index) {
    snprintf(time_text, sizeof(time_text), "%d:%02d", hof[index].min - 1,
             hof[index].tick / 12);
...
void __pascal far hof_write() {
  ...
  if (handle == NULL || fwrite(&hof_count, 1, 2, handle) != 2 ||
...
int __pascal far fade_in_1() {
  ...
  if (graphics_mode == gmMcgaVga) {
    fade_palette_buffer =
        make_pal_buffer_fadein(offscreen_surface, 0x6689, /*0*/ 2);
    is_global_fading = 1;
    do {
