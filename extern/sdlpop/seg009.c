...
char* get_current_filename_from_directory_listing(
    directory_listing_type* data) {
...
// seg009:0C90
void __pascal far restore_stuff() { SDL_Quit(); }
...
static FILE* open_dat_from_root_or_data_dir(const char* filename) {
  ...
  if (fp == NULL) {
    ...
    if (!file_exists(data_path)) {
      find_exe_dir();
      snprintf_check(data_path, sizeof(data_path), "%s/data/%s", exe_dir,
                     filename);
    }
...
dat_type* __pascal open_dat(const char* filename, int drive) {
  ...
  if (fp != NULL) {
    if (fread(&dat_header, 6, 1, fp) != 1) goto failed;
    dat_table = (dat_table_type*)malloc(dat_header.table_size);
    if (dat_table == NULL || fseek(fp, dat_header.table_offset, SEEK_SET) ||
        fread(dat_table, dat_header.table_size, 1, fp) != 1)
      goto failed;
    pointer->handle = fp;
    pointer->dat_table = dat_table;
  } else {
    /* // showmessage will crash if we call if before certain things are
    initialized!
    ...
    TITLE, KID, VPALACE, etc.) strncpy(filename_no_ext, pointer->filename,
    sizeof(filename_no_ext)); size_t len = strlen(filename_no_ext); if (len >= 5
    && filename_no_ext[len-4] == '.') { filename_no_ext[len-4] = '\0'; //
    terminate, so ".DAT" is deleted from the filename
    }
    ...
    if (result != 0 || !S_ISDIR(path_stat.st_mode)) {
            ...
            snprintf_check(error_message, sizeof(error_message), "Cannot find a
    required data file: %s\nPress any key to quit.", filename); if
    (onscreen_surface_ != NULL && copyprot_dialog != NULL) { // otherwise
    showmessage will crash showmessage(error_message, 1, &key_test_quit);
                    quit(1);
            }
    }
    */
...
// seg009:104E
chtab_type* __pascal load_sprites_from_file(int resource, int palette_bits,
                                            int quit_on_error) {
  int i;
  int n_images = 0;
  ...
  if (shpl == NULL) {
    ...
    if (quit_on_error) {
      ...
      snprintf_check(error_message, sizeof(error_message),
                     "Cannot find a required data file: %s\nMake sure that the "
                     "data/ folder exists.\nPress any key to quit.",
                     dat_chain_ptr->filename);
...
// seg009:8CE6
void __pascal far decompress_rle_lr(byte far* destination,
                                    const byte far* source, int dest_length) {
...
// seg009:8D1C
void __pascal far decompress_rle_ud(byte far* destination,
                                    const byte far* source, int dest_length,
...
// seg009:90FA
byte far* __pascal far decompress_lzg_lr(byte far* dest, const byte far* source,
                                         int dest_length) {
...
// seg009:91AD
byte far* __pascal far decompress_lzg_ud(byte far* dest, const byte far* source,
                                         int dest_length, int stride,
                                         int height) {
  byte* window = (byte*)malloc_near(0x400);
  if (window == NULL) return NULL;
  memset(window, 0, 0x400);
...
// seg009:938E
void __pascal far decompr_img(byte far* dest, const image_data_type far* source,
                              int decomp_size, int cmeth, int stride) {
  switch (cmeth) {
...
// seg009:121A
image_type* far __pascal far load_image(int resource_id,
  ...
  if (image != NULL) {
    ...
    if (SDL_SetSurfaceAlphaMod(image, 0) !=
        0) {  // sdl 1.2: SDL_SetAlpha removed
      sdlperror("load_image: SDL_SetAlpha");
      quit(1);
    }
    //		image_type* colored_image = SDL_ConvertSurfaceFormat(image,
    //SDL_PIXELFORMAT_ARGB8888, 0); 		if (!colored_image) { 			sdlperror("load_image:
    //SDL_ConvertSurfaceFormat"); 			quit(1);
    //		}
...
// seg009:13C4
void __pascal far draw_image_transp(image_type far* image, image_type far* mask,
                                    int xpos, int ypos) {
  if (graphics_mode == gmMcgaVga) {
...
int __pascal far set_joy_mode() {
  ...
  if (enable_controller_rumble && is_joyst_mode) {
    ...
    SDL_HapticRumbleInit(
        sdl_haptic);  // initialize the device for simple rumble
  } else {
    sdl_haptic = NULL;
  }
...
surface_type far* __pascal make_offscreen_buffer(const rect_type far* rect) {
  ...
  return SDL_CreateRGBSurface(0, rect->right, rect->bottom, 24, 0xFF, 0xFF << 8,
                              0xFF << 16, 0);  // RGB888 (little endian)
#else
  return SDL_CreateRGBSurface(0, rect->right, rect->bottom, 32, 0xFF, 0xFF << 8,
                              0xFF << 16, 0xFF << 24);
#endif
  // return surface;
}
...
// seg009:2288
void __pascal far draw_image_transp_vga(image_type far* image, int xpos,
                                        int ypos) {
  // stub
...
font_type load_font_from_data(/*const*/ rawfont_type* data) {
  ...
  chtab_type* chtab =
      malloc(sizeof(chtab_type) + sizeof(image_type * far) * n_chars);
  int chr, index;
  ...
  for (index = 0, chr = data->first_char; chr <= data->last_char;
       ++index, ++chr) {
    ...
    if (image_data->height == 0)
      image_data->height =
          1;  // HACK: decode_image() returns NULL if height==0.
    image_type* image;
...
// seg009:3E99
int __pascal far find_linebreak(const char far* text, int length,
                                int break_width, int x_align) {
...
// seg009:3F01
const rect_type far* __pascal draw_text(const rect_type far* rect_ptr,
                                        int x_align, int y_align,
                                        const char far* text, int length) {
...
// seg009:3E4F
void __pascal far show_text(const rect_type far* rect_ptr, int x_align,
                            int y_align, const char far* text) {
  // stub
  // printf("show_text: %s\n",text);
...
// seg009:04FF
void __pascal far show_text_with_color(const rect_type far* rect_ptr,
                                       int x_align, int y_align,
                                       const char far* text, int color) {
...
// seg009:08FB
dialog_type* __pascal far make_dialog_info(dialog_settings_type* settings,
                                           rect_type* dialog_rect,
                                           rect_type* text_rect,
  ...
  if (text_rect != NULL) dialog_info->text_rect = *text_rect;
...
void __pascal far dialog_method_2_frame(dialog_type* dialog) {
  ...
  rect.bottom = text_bottom + bottom_border -
                outer_border;  // (rect.right stays the same)
  draw_rect(&rect, color_15_brightwhite);
...
// seg009:053C
int __pascal far input_str(const rect_type far* rect, char* buffer,
                           int max_length, const char* initial, int has_initial,
                           int arg_4, int color, int bgcolor) {
  short length;
  word key;
...
// seg009:3985
surface_type far* __pascal rect_sthg(surface_type* surface,
                                     const rect_type far* rect) {
  // stub
  return surface;
}

// seg009:39CE
rect_type far* __pascal shrink2_rect(rect_type far* target_rect,
                                     const rect_type far* source_rect,
...
peel_type* __pascal far read_peel_from_screen(const rect_type far* rect) {
  ...
  result->rect = *rect;
#ifndef USE_ALPHA
  SDL_Surface* peel_surface = SDL_CreateRGBSurface(
      0, rect->right - rect->left, rect->bottom - rect->top, 24, 0xFF,
      0xFF << 8, 0xFF << 16, 0);
#else
  SDL_Surface* peel_surface = SDL_CreateRGBSurface(
      0, rect->right - rect->left, rect->bottom - rect->top, 32, 0xFF,
...
// seg009:3D95
int __pascal far intersect_rect(rect_type far* output,
                                const rect_type far* input1,
                                const rect_type far* input2) {
...
// seg009:4063
rect_type far* __pascal far union_rect(rect_type far* output,
                                       const rect_type far* input1,
                                       const rect_type far* input2) {
...
void speaker_callback(void* userdata, Uint8* stream, int len) {
  ...
  while (total_samples_left > 0) {
    ...
    if (note->frequency <= 0x01 /*rest*/) {
      ...
      generate_square_wave(stream, (float)note->frequency,
...
void init_digi() {
  ...
  if (version.major <= 2 && version.minor <= 0 && version.patch <= 3) {
    ...
    printf(
        "Your SDL.dll is older than 2.0.4. Using 8-bit audio format to work "
  ...
  if (enable_music && !digi_unavailable && result == NULL && index >= 0 &&
    ...
    if (sound_names != NULL && sound_name(index) != NULL) {
      ...
      do {
        ...
        stb_vorbis* decoder =
            stb_vorbis_open_memory(file_contents, file_size, NULL, NULL);
        if (decoder == NULL) {
          free(file_contents);
          break;
        }
...
void init_overlay() {
  ...
  if (!initialized) {
    overlay_surface = SDL_CreateRGBSurface(0, 320, 200, 32, 0xFF, 0xFF << 8,
                                           0xFF << 16, 0xFF << 24);
    merged_surface =
        SDL_CreateRGBSurface(0, 320, 200, 24, 0xFF, 0xFF << 8, 0xFF << 16, 0);
    initialized = true;
  }
}
...
void init_scaling() {
  ...
  if (scaling_type == 1) {
    if (!is_renderer_targettexture_supported && onscreen_surface_2x == NULL) {
      onscreen_surface_2x = SDL_CreateRGBSurface(0, 320 * 2, 200 * 2, 24, 0xFF,
                                                 0xFF << 8, 0xFF << 16, 0);
    }
...
void __pascal far set_gr_mode(byte grmode) {
  ...
  renderer_ = SDL_CreateRenderer(window_, -1,
                                 RENDER_BACKEND | SDL_RENDERER_TARGETTEXTURE);
...
#else
    printf(
        "Warning: You need to compile with SDL 2.0.5 or newer for the "
  ...
  //SDL_DEFAULT_REPEAT_INTERVAL) != 0) {  //deprecated 		sdlperror("set_gr_mode:
  //SDL_EnableKeyRepeat"); 		quit(1);
  //	}
...
// seg009:9C6C
int __pascal far get_text_color(int cga_color, int low_half,
                                int high_half_mask) {
...
void load_from_opendats_metadata(int resource_id, const char* extension,
  ...
  *result = data_none;
  ...
  for (pointer = dat_chain_ptr; fp == NULL && pointer != NULL;
...
// seg009:9F80
void far* __pascal load_from_opendats_alloc(int resource, const char* extension,
                                            data_location* out_result,
                                            int* out_size) {
  // stub
  // printf("id = %d\n",resource);
  dat_type* pointer;
...
// seg009:A172
int __pascal far load_from_opendats_to_area(int resource, void far* area,
                                            int length, const char* extension) {
  // stub
  // return 0;
  dat_type* pointer;
  data_location result;
...
void __pascal far method_1_blit_rect(surface_type near* target_surface,
                                     surface_type near* source_surface,
                                     const rect_type far* target_rect,
                                     const rect_type far* source_rect,
                                     int blit) {
  SDL_Rect src_rect;
...
image_type far* __pascal far method_3_blit_mono(image_type far* image, int xpos,
                                                int ypos, int blitter,
                                                byte color) {
  int w = image->w;
  int h = image->h;
  if (SDL_SetColorKey(image, SDL_TRUE, 0) != 0) {
...
const rect_type far* __pascal far method_5_rect(const rect_type far* rect,
                                                int blit, byte color) {
  SDL_Rect dest_rect;
...
void blit_xor(SDL_Surface* target_surface, SDL_Rect* dest_rect,
  ...
  SDL_Surface* helper_surface = SDL_CreateRGBSurface(
      0, dest_rect->w, dest_rect->h, 24, 0xFF, 0xFF << 8, 0xFF << 16, 0);
  ...
  // SDL_CreateRGBSurface(0, src_rect->w, src_rect->h, 24, 0xFF, 0xFF<<8,
  // 0xFF<<16, 0);
  if (image_24 == NULL) {
    sdlperror("blit_xor: SDL_CreateRGBSurface");
    quit(1);
...
image_type far* __pascal far method_6_blit_img_to_scr(image_type far* image,
                                                      int xpos, int ypos,
                                                      int blit) {
  if (image == NULL) {
    ...
    switch (event.type) {
      case SDL_KEYDOWN: {
        ...
        if (escape_key_suppressed &&
          ...
          break;  // Prevent repeated keystrokes opening/closing the menu as
                  // long as the key is held down.
        } else if ((modifier & KMOD_ALT) && scancode == SDL_SCANCODE_RETURN) {
          ...
          switch (scancode) {
            ...
            default:
              // If Alt is held down from Alt+Tab: ignore it until it's
              // released.
      ...
      case SDL_KEYUP:
        // If Alt was held down from Alt+Tab but now it's released: stop
        // ignoring Tab.
        ...
        // Prevent repeated keystrokes opening/closing the menu as long as the
        // key is held down.
      ...
      case SDL_JOYAXISMOTION:
        ...
        if (event.type == SDL_JOYBUTTONDOWN) {
          if (event.jbutton.button == SDL_JOYSTICK_BUTTON_Y)
            joy_AY_buttons_state = -1;  // Y (up)
          else if (event.jbutton.button == SDL_JOYSTICK_BUTTON_X)
            joy_X_button_state = -1;  // X (shift)
        } else if (event.type == SDL_JOYBUTTONUP) {
          if (event.jbutton.button == SDL_JOYSTICK_BUTTON_Y)
            joy_AY_buttons_state = 0;  // Y (up)
          else if (event.jbutton.button == SDL_JOYSTICK_BUTTON_X)
            joy_X_button_state = 0;  // X (shift)
        }
        break;

      case SDL_TEXTINPUT:
        last_text_input =
            event.text.text[0];  // UTF-8 formatted char text input
        break;
        ...
        */
        switch (event.window.event) {
          ...
          case SDL_WINDOWEVENT_FOCUS_GAINED:
            // Fix for this bug: When playing back a recording, Alt+Tabbing back
            // to SDLPoP stops the replay if Alt is released before Tab.
            {  // If Alt is held down from Alt+Tab: ignore it until it's
               // released.
...
// seg009:07EB
rect_type far* __pascal offset4_rect_add(rect_type far* dest,
                                         const rect_type far* source,
                                         int d_left, int d_top, int d_right,
                                         int d_bottom) {
  *dest = *source;
  dest->left += d_left;
  dest->top += d_top;
...
// seg009:3AA5
rect_type far* __pascal offset2_rect(rect_type far* dest,
                                     const rect_type far* source, int delta_x,
                                     int delta_y) {
...
palette_fade_type far* __pascal make_pal_buffer_fadein(
  ...
  memcpy_far(palette_buffer->faded_pal, palette_buffer->original_pal,
             sizeof(palette_buffer->faded_pal));
  var_8 = 0;
  for (curr_row = 0, curr_row_mask = 1; curr_row < 0x10;
       ++curr_row, curr_row_mask <<= 1) {
    if (which_rows & curr_row_mask) {
      memset_far(palette_buffer->faded_pal + (curr_row << 4), 0,
...
// seg009:1B64
void __pascal far pal_restore_free_fadein(
    palette_fade_type far* palette_buffer) {
...
int __pascal far fade_in_frame(palette_fade_type far* palette_buffer) {
  ...
  for (start = 0, current_row_mask = 1; start < 0x100;
...
// seg009:1D28
palette_fade_type far* __pascal make_pal_buffer_fadeout(int which_rows,
                                                        int wait_time) {
  palette_fade_type far* palette_buffer;
  palette_buffer = (palette_fade_type*)malloc_far(sizeof(palette_fade_type));
  palette_buffer->which_rows = which_rows;
...
// seg009:1DAF
void __pascal far pal_restore_free_fadeout(
    palette_fade_type far* palette_buffer) {
...
int has_timer_stopped(int timer_index) {
  ...
  if (overshoot >= 0) {
    ...
    //timer_last_counter[timer_index]) * milliseconds_per_counter; 		printf("timer
    //%d:   frametime (ms) = %5.1f    fps = %.1f    timer ticks elapsed = %d\n",
