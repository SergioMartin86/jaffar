...
void load_arrowhead_images() {
  ...
  if (arrowhead_up_image == NULL) {
    arrowhead_up_image =
        decode_image((image_data_type*)arrowhead_up_image_data, &dat_pal);
  }
  if (arrowhead_down_image == NULL) {
    arrowhead_down_image =
        decode_image((image_data_type*)arrowhead_down_image_data, &dat_pal);
  }
  //	dat_pal.vga[1] = vga_palette[color_7_lightgray];
  if (arrowhead_left_image == NULL) {
    arrowhead_left_image =
        decode_image((image_data_type*)arrowhead_left_image_data, &dat_pal);
  }
  if (arrowhead_right_image == NULL) {
    arrowhead_right_image =
        decode_image((image_data_type*)arrowhead_right_image_data, &dat_pal);
  }
}
...
pause_menu_item_type pause_menu_items[] = {
    ...
    /*{.id = PAUSE_MENU_CHEATS,        .text = "CHEATS", .required =
       &cheats_enabled},*/
...
setting_type general_settings[] = {
    {.id = SETTING_SHOW_MENU_ON_PAUSE,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &enable_pause_menu,
    ...
    {.id = SETTING_ENABLE_INFO_SCREEN,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &enable_info_screen,
     .text = "Display info screen on launch",
     .explanation =
         "Display the SDLPoP information screen when the game starts."},
    {.id = SETTING_ENABLE_SOUND,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &is_sound_on,
     .text = "Enable sound",
     .explanation = "Turn sound on or off."},
    {.id = SETTING_ENABLE_MUSIC,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &enable_music,
     .text = "Enable music",
     .explanation = "Turn music on or off."},
    {.id = SETTING_ENABLE_CONTROLLER_RUMBLE,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &enable_controller_rumble,
     .text = "Enable controller rumble",
     .explanation = "If using a controller with a rumble motor, provide haptic "
                    "feedback when the kid is hurt."},
    {.id = SETTING_JOYSTICK_THRESHOLD,
     .style = SETTING_STYLE_NUMBER,
     .number_type = SETTING_INT,
     .linked = &joystick_threshold,
     .min = 0,
     .max = INT16_MAX,
     .text = "Joystick threshold",
     .explanation = "Joystick 'dead zone' sensitivity threshold."},
    {.id = SETTING_JOYSTICK_ONLY_HORIZONTAL,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &joystick_only_horizontal,
    ...
    {.id = SETTING_RESET_ALL_SETTINGS,
     .style = SETTING_STYLE_TEXT_ONLY,
     .text = "Restore defaults...",
     .explanation = "Revert all settings to the default state."},
};

NAMES_LIST(scaling_type_setting_names, {
                                           "Sharp",
                                           "Fuzzy",
                                           "Blurry",
                                       });
...
setting_type visuals_settings[] = {
    {.id = SETTING_FULLSCREEN,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &start_fullscreen,
     .text = "Start fullscreen",
     .explanation = "Start the game in fullscreen mode.\nYou can also toggle "
                    "fullscreen by pressing Alt+Enter."},
    {.id = SETTING_USE_CORRECT_ASPECT_RATIO,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &use_correct_aspect_ratio,
    ...
    {.id = SETTING_USE_INTEGER_SCALING,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &use_integer_scaling,
     .required = &integer_scaling_possible,
     .text = "Use integer scaling",
     .explanation =
         "Enable pixel perfect scaling. That is, make all pixels the same size "
         "by forcing integer scale factors.\n"
         "Combining with 4:3 aspect ratio requires at least 1600x1200."
         "\nYou need to compile with SDL 2.0.5 or newer to enable this."},
    {.id = SETTING_SCALING_TYPE,
     .style = SETTING_STYLE_NUMBER,
     .number_type = SETTING_BYTE,
     .max = 2,
     .linked = &scaling_type,
     .names_list = &scaling_type_setting_names_list,
     .text = "Scaling method",
     .explanation =
    ...
    {.id = SETTING_ENABLE_FADE,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &enable_fade,
     .text = "Fading enabled",
     .explanation = "Turn fading on or off."},
    {.id = SETTING_ENABLE_LIGHTING,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &enable_lighting,
...
setting_type gameplay_settings[] = {
    {.id = SETTING_ENABLE_CHEATS, .style = SETTING_STYLE_TOGGLE, .linked = &cheats_enabled, .text = "Enable cheats", .explanation = "Turn cheats on or off." /*"\nAlso, display the CHEATS option on the pause menu."*/},
    {.id = SETTING_ENABLE_QUICKSAVE,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &enable_quicksave,
     .text = "Enable quicksave",
     .explanation = "Enable quicksave/load feature.\nPress F6 to quicksave, F9 "
                    "to quickload."},
    {.id = SETTING_ENABLE_QUICKSAVE_PENALTY,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &enable_quicksave_penalty,
    ...
    {.id = SETTING_ENABLE_REPLAY,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &enable_replay,
     .text = "Enable replays",
     .explanation = "Enable recording/replay feature.\n"
                    "Press Ctrl+Tab in-game to start recording.\n"
                    "To stop, press Ctrl+Tab again."},
    {.id = SETTING_USE_FIXES_AND_ENHANCEMENTS,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &use_fixes_and_enhancements,
    ...
    {.id = SETTING_ENABLE_CROUCH_AFTER_CLIMBING,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.enable_crouch_after_climbing,
     .required = &use_fixes_and_enhancements,
     .text = "Enable crouching after climbing",
     .explanation = "Adds a way to crouch immediately after climbing up: press "
                    "down and forward simultaneously. "
                    "In the original game, this could not be done (pressing "
                    "down always causes the kid to climb down)."},
    {.id = SETTING_ENABLE_FREEZE_TIME_DURING_END_MUSIC,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.enable_freeze_time_during_end_music,
     .required = &use_fixes_and_enhancements,
     .text = "Freeze time during level end music",
     .explanation = "Time runs out while the level ending music plays; "
                    "however, the music can be skipped by disabling sound. "
                    "This option stops time while the ending music is playing "
                    "(so there is no need to disable sound)."},
    {.id = SETTING_ENABLE_REMEMBER_GUARD_HP,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.enable_remember_guard_hp,
     .required = &use_fixes_and_enhancements,
     .text = "Remember guard hitpoints",
     .explanation = "Enable guard hitpoints not resetting to their default "
                    "(maximum) value when re-entering the room."},
    {.id = SETTING_FIX_GATE_SOUNDS,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_gate_sounds,
     .required = &use_fixes_and_enhancements,
     .text = "Fix gate sounds bug",
     .explanation = "If a room is linked to itself on the left, the closing "
                    "sounds of the gates in that room can't be heard."},
    {.id = SETTING_TWO_COLL_BUG,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_two_coll_bug,
     .required = &use_fixes_and_enhancements,
     .text = "Fix two collisions bug",
     .explanation = "An open gate or chomper may enable the Kid to go through "
                    "walls. (Trick 7, 37, 62)"},
    {.id = SETTING_FIX_INFINITE_DOWN_BUG,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_infinite_down_bug,
     .required = &use_fixes_and_enhancements,
     .text = "Fix infinite down bug",
     .explanation = "If a room is linked to itself at the bottom, and the "
                    "Kid's column has no floors, the game hangs."},
    {.id = SETTING_FIX_GATE_DRAWING_BUG,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_gate_drawing_bug,
     .required = &use_fixes_and_enhancements,
     .text = "Fix gate drawing bug",
     .explanation = "When a gate is under another gate, the top of the bottom "
                    "gate is not visible."},
    {.id = SETTING_FIX_BIGPILLAR_CLIMB,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_bigpillar_climb,
     .required = &use_fixes_and_enhancements,
     .text = "Fix big pillar climbing bug",
     .explanation = "When climbing up to a floor with a big pillar top behind, "
                    "turned right, Kid sees through floor."},
    {.id = SETTING_FIX_JUMP_DISTANCE_AT_EDGE,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_jump_distance_at_edge,
     .required = &use_fixes_and_enhancements,
     .text = "Fix jump distance at edge",
     .explanation = "When climbing up two floors, turning around and jumping "
                    "upward, the kid falls down. "
                    "This fix makes the workaround of Trick 25 unnecessary."},
    {.id = SETTING_FIX_EDGE_DISTANCE_CHECK_WHEN_CLIMBING,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_edge_distance_check_when_climbing,
     .required = &use_fixes_and_enhancements,
     .text = "Fix edge distance check when climbing",
     .explanation = "When climbing to a higher floor, the game unnecessarily "
                    "checks how far away the edge below is. "
                    "Sometimes you will \"teleport\" some distance when "
                    "climbing from firm ground."},
    {.id = SETTING_FIX_PAINLESS_FALL_ON_GUARD,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_painless_fall_on_guard,
     .required = &use_fixes_and_enhancements,
     .text = "Fix painless fall on guard",
     .explanation = "Falling from a great height directly on top of guards "
                    "does not hurt."},
    {.id = SETTING_FIX_WALL_BUMP_TRIGGERS_TILE_BELOW,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_wall_bump_triggers_tile_below,
     .required = &use_fixes_and_enhancements,
     .text = "Fix wall bump triggering tile below",
     .explanation =
         "Bumping against a wall may cause a loose floor below to drop, even "
         "though it has not been touched. (Trick 18, 34)"},
    {.id = SETTING_FIX_STAND_ON_THIN_AIR,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_stand_on_thin_air,
     .required = &use_fixes_and_enhancements,
     .text = "Fix standing on thin air",
     .explanation = "When pressing a loose tile, you can temporarily stand on "
                    "thin air by standing up from crouching."},
    {.id = SETTING_FIX_PRESS_THROUGH_CLOSED_GATES,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_press_through_closed_gates,
     .required = &use_fixes_and_enhancements,
     .text = "Fix pressing through closed gates",
     .explanation = "Buttons directly to the right of gates can be pressed "
                    "even though the gate is closed (Trick 1)"},
    {.id = SETTING_FIX_GRAB_FALLING_SPEED,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_grab_falling_speed,
     .required = &use_fixes_and_enhancements,
     .text = "Fix grab falling speed",
     .explanation =
         "By jumping and bumping into a wall, you can sometimes grab a ledge "
         "two stories down (which should not be possible)."},
    {.id = SETTING_FIX_SKELETON_CHOMPER_BLOOD,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_skeleton_chomper_blood,
     .required = &use_fixes_and_enhancements,
     .text = "Fix skeleton chomper blood",
     .explanation = "When chomped, skeletons cause the chomper to become "
                    "bloody even though skeletons do not have blood."},
    {.id = SETTING_FIX_MOVE_AFTER_DRINK,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_move_after_drink,
     .required = &use_fixes_and_enhancements,
     .text = "Fix movement after drinking",
     .explanation = "Controls do not get released properly when drinking a "
                    "potion, sometimes causing unintended movements."},
    {.id = SETTING_FIX_LOOSE_LEFT_OF_POTION,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_loose_left_of_potion,
     .required = &use_fixes_and_enhancements,
     .text = "Fix loose floor left of potion",
     .explanation = "A drawing bug occurs when a loose tile is placed to the "
                    "left of a potion (or sword)."},
    {.id = SETTING_FIX_GUARD_FOLLOWING_THROUGH_CLOSED_GATES,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_guard_following_through_closed_gates,
     .required = &use_fixes_and_enhancements,
     .text = "Fix guards passing closed gates",
     .explanation = "Guards may \"follow\" the kid to the room on the left or "
                    "right, even though there is a closed gate in between."},
    {.id = SETTING_FIX_SAFE_LANDING_ON_SPIKES,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_safe_landing_on_spikes,
     .required = &use_fixes_and_enhancements,
     .text = "Fix safe landing on spikes",
     .explanation = "When landing on the edge of a spikes tile, it is "
                    "considered safe. (Trick 65)"},
    {.id = SETTING_FIX_GLIDE_THROUGH_WALL,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_glide_through_wall,
     .required = &use_fixes_and_enhancements,
     .text = "Fix gliding through walls",
     .explanation = "The kid may glide through walls after turning around "
                    "while running (especially when weightless)."},
    {.id = SETTING_FIX_DROP_THROUGH_TAPESTRY,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_drop_through_tapestry,
     .required = &use_fixes_and_enhancements,
     .text = "Fix dropping through tapestries",
     .explanation = "The kid can drop down through a closed gate, when there "
                    "is a tapestry (doortop) above the gate."},
    {.id = SETTING_FIX_LAND_AGAINST_GATE_OR_TAPESTRY,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_land_against_gate_or_tapestry,
     .required = &use_fixes_and_enhancements,
     .text = "Fix land against gate or tapestry",
     .explanation = "When dropping down and landing right in front of a wall, "
                    "the entire landing animation should normally play. "
                    "However, when falling against a closed gate or a "
                    "tapestry(+floor) tile, the animation aborts."},
    {.id = SETTING_FIX_UNINTENDED_SWORD_STRIKE,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_unintended_sword_strike,
     .required = &use_fixes_and_enhancements,
     .text = "Fix unintended sword strike",
     .explanation = "Sometimes, the kid may automatically strike immediately "
                    "after drawing the sword. "
                    "This especially happens when dropping down from a higher "
                    "floor and then turning towards the opponent."},
    {.id = SETTING_FIX_RETREAT_WITHOUT_LEAVING_ROOM,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_retreat_without_leaving_room,
     .required = &use_fixes_and_enhancements,
     .text = "Fix retreat without leaving room",
     .explanation =
         "By repeatedly pressing 'back' in a swordfight, you can retreat out "
         "of a room without the room changing. (Trick 35)"},
    {.id = SETTING_FIX_RUNNING_JUMP_THROUGH_TAPESTRY,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_running_jump_through_tapestry,
     .required = &use_fixes_and_enhancements,
     .text = "Fix running jumps through tapestries",
     .explanation = "The kid can jump through a tapestry with a running jump "
                    "to the left, if there is a floor above it."},
    {.id = SETTING_FIX_PUSH_GUARD_INTO_WALL,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_push_guard_into_wall,
     .required = &use_fixes_and_enhancements,
     .text = "Fix pushing guards into walls",
     .explanation = "Guards can be pushed into walls, because the game does "
                    "not correctly check for walls located behind a guard."},
    {.id = SETTING_FIX_JUMP_THROUGH_WALL_ABOVE_GATE,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_jump_through_wall_above_gate,
     .required = &use_fixes_and_enhancements,
     .text = "Fix jump through wall above gate",
     .explanation = "By doing a running jump into a wall, you can fall behind "
                    "a closed gate two floors down. (e.g. skip in Level 7)"},
    {.id = SETTING_FIX_CHOMPERS_NOT_STARTING,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_chompers_not_starting,
     .required = &use_fixes_and_enhancements,
     .text = "Fix chompers not starting",
     .explanation = "If you grab a ledge that is one or more floors down, the "
                    "chompers on that row will not start."},
    {.id = SETTING_FIX_FEATHER_INTERRUPTED_BY_LEVELDOOR,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_feather_interrupted_by_leveldoor,
     .required = &use_fixes_and_enhancements,
     .text = "Fix leveldoor interrupting feather fall",
     .explanation =
         "As soon as a level door has completely opened, the feather fall "
         "effect is interrupted because the sound stops."},
    {.id = SETTING_FIX_OFFSCREEN_GUARDS_DISAPPEARING,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_offscreen_guards_disappearing,
     .required = &use_fixes_and_enhancements,
     .text = "Fix offscreen guards disappearing",
     .explanation = "Guards will often not reappear in another room if they "
                    "have been pushed (partly or entirely) offscreen."},
    {.id = SETTING_FIX_MOVE_AFTER_SHEATHE,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_move_after_sheathe,
     .required = &use_fixes_and_enhancements,
     .text = "Fix movement after sheathing",
     .explanation = "While putting the sword away, if you press forward and "
                    "down, and then release down, the kid will still duck."},
    {.id = SETTING_FIX_HIDDEN_FLOORS_DURING_FLASHING,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_hidden_floors_during_flashing,
     .required = &use_fixes_and_enhancements,
     .text = "Fix hidden floors during flashing",
     .explanation = "After uniting with the shadow in level 12, the hidden "
                    "floors will not appear until after the flashing stops."},
    {.id = SETTING_FIX_HANG_ON_TELEPORT,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_hang_on_teleport,
     .required = &use_fixes_and_enhancements,
     .text = "Fix hang on teleport bug",
     .explanation =
         "By jumping towards one of the bottom corners of the room and "
         "grabbing a ledge, you can teleport to the room above."},
    {.id = SETTING_FIX_EXIT_DOOR,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &fixes_saved.fix_exit_door,
     .required = &use_fixes_and_enhancements,
     .text = "Fix exit doors",
     .explanation =
         "You can enter closed exit doors after you met the shadow or Jaffar "
         "died, or after you opened one of multiple exits."},
};
...
KEY_VALUE_LIST(direction_setting_names,
               {{"Left", dir_FF_left}, {"Right", dir_0_right}});
extern names_list_type never_is_16_list;

setting_type mods_settings[] = {
    {.id = SETTING_USE_CUSTOM_OPTIONS,
     .style = SETTING_STYLE_TOGGLE,
     .linked = &use_custom_options,
     .text = "Use customization options",
     .explanation = "Turn customization options on or off.\n(default = OFF)"},
    {.id = SETTING_LEVEL_SETTINGS,
     .style = SETTING_STYLE_TEXT_ONLY,
     .required = &use_custom_options,
     .text = "Customize level...",
     .explanation = "Change level-specific options (such as level type, guard "
                    "type, number of guard hitpoints)."},
    {.id = SETTING_START_MINUTES_LEFT,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.start_minutes_left,
     .number_type = SETTING_SHORT,
     .min = -1,
     .max = INT16_MAX,
     .text = "Starting minutes left",
     .explanation = "Starting minutes left. (default = 60)\n"
                    "To disable the time limit completely, set this to -1."},
    {.id = SETTING_START_TICKS_LEFT,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.start_ticks_left,
     .number_type = SETTING_WORD,
     .max = UINT16_MAX,
     .text = "Starting seconds left",
     .explanation = "Starting number of seconds left in the first "
                    "minute.\n(default = 59.92)"},
    {.id = SETTING_START_HITP,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.start_hitp,
     .number_type = SETTING_WORD,
     .max = UINT16_MAX,
     .text = "Starting hitpoints",
     .explanation = "Starting hitpoints. (default = 3)"},
    {.id = SETTING_MAX_HITP_ALLOWED,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.max_hitp_allowed,
     .number_type = SETTING_WORD,
     .max = UINT16_MAX,
     .text = "Max hitpoints allowed",
     .explanation = "Maximum number of hitpoints you can get. (default = 10)"},
    {.id = SETTING_SAVING_ALLOWED_FIRST_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.saving_allowed_first_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Saving allowed: first level",
     .explanation = "First level where you can save the game. (default = 3)"},
    {.id = SETTING_SAVING_ALLOWED_LAST_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.saving_allowed_last_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Saving allowed: last level",
     .explanation = "Last level where you can save the game. (default = 13)"},
    {.id = SETTING_START_UPSIDE_DOWN,
     .style = SETTING_STYLE_TOGGLE,
     .required = &use_custom_options,
     .linked = &custom_saved.start_upside_down,
     .text = "Start with the screen flipped",
     .explanation = "Start the game with the screen flipped upside down, "
                    "similar to Shift+I (default = OFF)"},
    {.id = SETTING_START_IN_BLIND_MODE,
     .style = SETTING_STYLE_TOGGLE,
     .required = &use_custom_options,
    ...
    {.id = SETTING_COPYPROT_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.copyprot_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Copy protection before level",
     .explanation =
         "The potions level will appear before this level. (default = 2)"},
    {.id = SETTING_DRAWN_TILE_TOP_LEVEL_EDGE,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .names_list = &tile_type_setting_names_list,
     .linked = &custom_saved.drawn_tile_top_level_edge,
     .number_type = SETTING_BYTE,
     .max = 31,
     .text = "Drawn tile: top level edge",
     .explanation = "Tile drawn at the top of the room if there is no room "
                    "that way. (default = floor)"},
    {.id = SETTING_DRAWN_TILE_LEFT_LEVEL_EDGE,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .names_list = &tile_type_setting_names_list,
     .linked = &custom_saved.drawn_tile_left_level_edge,
     .number_type = SETTING_BYTE,
     .max = 31,
     .text = "Drawn tile: left level edge",
     .explanation = "Tile drawn at the left of the room if there is no room "
                    "that way. (default = wall)"},
    {.id = SETTING_LEVEL_EDGE_HIT_TILE,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .names_list = &tile_type_setting_names_list,
     .linked = &custom_saved.level_edge_hit_tile,
     .number_type = SETTING_BYTE,
     .max = 31,
     .text = "Level edge hit tile",
     .explanation = "Tile behavior at the top or left of the room if there is "
                    "no room that way (default = wall)"},
    {.id = SETTING_ALLOW_TRIGGERING_ANY_TILE,
     .style = SETTING_STYLE_TOGGLE,
     .required = &use_custom_options,
     .linked = &custom_saved.allow_triggering_any_tile,
     .text = "Allow triggering any tile",
     .explanation =
         "Enable triggering any tile. For example a button could make loose "
         "floors fall, or start a stuck chomper. (default = OFF)"},
    {.id = SETTING_ENABLE_WDA_IN_PALACE,
     .style = SETTING_STYLE_TOGGLE,
     .required = &use_custom_options,
     ...
     .explanation = "Enable the dungeon wall drawing algorithm in the palace."
                    "\nN.B. Use with a modified VPALACE.DAT that provides "
                    "dungeon-like wall graphics! (default = OFF)"},
    {.id = SETTING_FIRST_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.first_level,
     .number_type = SETTING_WORD,
     .max = 15,
     .text = "First level",
     .explanation = "Level that will be loaded when starting a new game."
                    "\n(default = 1)"},
    {.id = SETTING_SKIP_TITLE,
     .style = SETTING_STYLE_TOGGLE,
     .required = &use_custom_options,
     .linked = &custom_saved.skip_title,
     .text = "Skip title sequence",
     .explanation = "Always skip the title sequence: the first level will be "
                    "loaded immediately."
                    "\n(default = OFF)"},
    {.id = SETTING_SHIFT_L_ALLOWED_UNTIL_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.shift_L_allowed_until_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Shift+L allowed until level",
     .explanation = "First level where level skipping with Shift+L is denied "
                    "in non-cheat mode.\n"
                    "(default = 4)"},
    {.id = SETTING_SHIFT_L_REDUCED_MINUTES,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.shift_L_reduced_minutes,
     .number_type = SETTING_WORD,
     .max = UINT16_MAX,
     .text = "Minutes left after Shift+L used",
     .explanation =
         "Number of minutes left after Shift+L is used in non-cheat mode.\n"
         "(default = 15)"},
    {.id = SETTING_SHIFT_L_REDUCED_TICKS,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.shift_L_reduced_ticks,
     .number_type = SETTING_WORD,
     .max = UINT16_MAX,
     .text = "Seconds left after Shift+L used",
     .explanation = "Number of seconds left after Shift+L is used in non-cheat "
                    "mode.\n(default = 59.92)"},
    {.id = SETTING_DEMO_HITP,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.demo_hitp,
     .number_type = SETTING_WORD,
     .max = UINT16_MAX,
     .text = "Demo level hitpoints",
     .explanation = "Hitpoints the kid has on the demo level.\n(default = 4)"},
    {.id = SETTING_DEMO_END_ROOM,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.demo_end_room,
     .number_type = SETTING_WORD,
     .min = 1,
     .max = 24,
     .text = "Demo level ending room",
     .explanation = "Demo level ending room.\n(default = 24)"},
    {.id = SETTING_INTRO_MUSIC_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.intro_music_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Level with intro music",
     .explanation = "Level where the presentation music is played when the kid "
                    "crouches down. (default = 1)\n"
                    "Note: only works if this level is the starting level."},
    {.id = SETTING_HAVE_SWORD_FROM_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.have_sword_from_level,
     .number_type = SETTING_WORD,
     .min = 1,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Have sword from level",
     .explanation = "First level (except the demo level) where kid has the "
                    "sword.\n(default = 2)\n"},
    {.id = SETTING_CHECKPOINT_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.checkpoint_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
    ...
    {.id = SETTING_CHECKPOINT_RESPAWN_DIR,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.checkpoint_respawn_dir,
     .number_type = SETTING_SBYTE,
     .min = -1,
     .max = 0,
     .names_list = &direction_setting_names_list,
     .text = "Checkpoint respawn direction",
     .explanation = "Respawn direction after triggering the "
                    "checkpoint.\n(default = left)"},
    {.id = SETTING_CHECKPOINT_RESPAWN_ROOM,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.checkpoint_respawn_room,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     .text = "Checkpoint respawn room",
     .explanation = "Room where you respawn after triggering the "
                    "checkpoint.\n(default = 2)"},
    {.id = SETTING_CHECKPOINT_RESPAWN_TILEPOS,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.checkpoint_respawn_tilepos,
     .number_type = SETTING_BYTE,
     .max = 29,
     .text = "Checkpoint respawn tile position",
     .explanation = "Tile position (0 to 29) where you respawn after "
                    "triggering the checkpoint.\n(default = 6)"},
    {.id = SETTING_CHECKPOINT_CLEAR_TILE_ROOM,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.checkpoint_clear_tile_room,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     .text = "Checkpoint clear tile room",
     .explanation = "Room where a tile is cleared after respawning at the "
                    "checkpoint location.\n(default = 7)"},
    {.id = SETTING_CHECKPOINT_CLEAR_TILE_COL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.checkpoint_clear_tile_col,
     .number_type = SETTING_BYTE,
     .max = 9,
     .text = "Checkpoint clear tile column",
     .explanation =
         "Location (column/row) of the cleared tile after respawning at the "
         "checkpoint location.\n(default: column = 4, row = top)"},
    {.id = SETTING_CHECKPOINT_CLEAR_TILE_ROW,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.checkpoint_clear_tile_row,
     .number_type = SETTING_BYTE,
     .max = 2,
     .names_list = &row_setting_names_list,
     .text = "Checkpoint clear tile row",
     .explanation =
         "Location (column/row) of the cleared tile after respawning at the "
         "checkpoint location.\n(default: column = 4, row = top)"},
    {.id = SETTING_SKELETON_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Skeleton awakes level",
     .explanation = "Level and room where a skeleton can come "
                    "alive.\n(default: level = 3, room = 1)"},
    {.id = SETTING_SKELETON_ROOM,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_room,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     ...
     .explanation = "Level and room where a skeleton can come "
                    "alive.\n(default: level = 3, room = 1)"},
    {.id = SETTING_SKELETON_TRIGGER_COLUMN_1,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_trigger_column_1,
     .number_type = SETTING_BYTE,
     .max = 9,
     .text = "Skeleton trigger column (1)",
     .explanation = "The skeleton will wake up if the kid is on one of these "
                    "two columns.\n(defaults = 2,3)"},
    {.id = SETTING_SKELETON_TRIGGER_COLUMN_2,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_trigger_column_2,
     .number_type = SETTING_BYTE,
     .max = 9,
     .text = "Skeleton trigger column (2)",
     .explanation = "The skeleton will wake up if the kid is on one of these "
                    "two columns.\n(defaults = 2,3)"},
    {.id = SETTING_SKELETON_COLUMN,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_column,
     .number_type = SETTING_BYTE,
     .max = 9,
     .text = "Skeleton tile column",
     .explanation = "Location (column/row) of the skeleton tile that will "
                    "awaken.\n(default: column = 5, row = middle)"},
    {.id = SETTING_SKELETON_ROW,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_row,
     .number_type = SETTING_BYTE,
     .max = 2,
     .names_list = &row_setting_names_list,
     ...
     .explanation = "Location (column/row) of the skeleton tile that will "
                    "awaken.\n(default: column = 5, row = middle)"},
    {.id = SETTING_SKELETON_REQUIRE_OPEN_LEVEL_DOOR,
     .style = SETTING_STYLE_TOGGLE,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_require_open_level_door,
     .text = "Skeleton requires level door",
     .explanation = "Whether the level door must first be opened before the "
                    "skeleton awakes.\n(default = true)"},
    {.id = SETTING_SKELETON_SKILL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_skill,
     .number_type = SETTING_BYTE,
     .max = 15,
     .text = "Skeleton skill",
     .explanation = "Skill of the awoken skeleton.\n(default = 2)"},
    {.id = SETTING_SKELETON_REAPPEAR_ROOM,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_reappear_room,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     .text = "Skeleton reappear room",
     .explanation = "If the skeleton falls into this room, it will reappear "
                    "there.\n(default = 3)"},
    {.id = SETTING_SKELETON_REAPPEAR_X,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_reappear_x,
     .number_type = SETTING_BYTE,
     .max = 255,
     .text = "Skeleton reappear X coordinate",
     .explanation = "Horizontal coordinate where the skeleton "
                    "reappears.\n(default = 133)\n"
                    "(58 = left edge of the room, 198 = right edge)"},
    {.id = SETTING_SKELETON_REAPPEAR_ROW,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_reappear_row,
     .number_type = SETTING_BYTE,
     .max = 2,
     .names_list = &row_setting_names_list,
     .text = "Skeleton reappear row",
     .explanation = "Row on which the skeleton reappears.\n(default = middle)"},
    {.id = SETTING_SKELETON_REAPPEAR_DIR,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.skeleton_reappear_dir,
     .number_type = SETTING_SBYTE,
     .min = -1,
     .max = 0,
     .names_list = &direction_setting_names_list,
     .text = "Skeleton reappear direction",
     .explanation = "Direction the skeleton is facing when it "
                    "reappears.\n(default = right)"},
    {.id = SETTING_MIRROR_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mirror_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Mirror level",
     .explanation = "Level and room where the mirror appears.\n(default: level "
                    "= 4, room = 4)"},
    {.id = SETTING_MIRROR_ROOM,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mirror_room,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     .text = "Mirror room",
     .explanation = "Level and room where the mirror appears.\n(default: level "
                    "= 4, room = 4)"},
    {.id = SETTING_MIRROR_COLUMN,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mirror_column,
     .number_type = SETTING_BYTE,
     .max = 9,
     .text = "Mirror column",
     .explanation = "Location (column/row) of the tile where the mirror "
                    "appears.\n(default: column = 4, row = top)"},
    {.id = SETTING_MIRROR_ROW,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mirror_row,
     .number_type = SETTING_BYTE,
     .max = 2,
     .names_list = &row_setting_names_list,
     .text = "Mirror row",
     .explanation = "Location (column/row) of the tile where the mirror "
                    "appears.\n(default: column = 4, row = top)"},
    {.id = SETTING_MIRROR_TILE,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mirror_tile,
     .number_type = SETTING_BYTE,
     .max = 31,
     .names_list = &tile_type_setting_names_list,
     .text = "Mirror tile",
     .explanation = "Tile type that appears when the mirror should "
                    "appear.\n(default = mirror)"},
    {.id = SETTING_SHOW_MIRROR_IMAGE,
     .style = SETTING_STYLE_TOGGLE,
     .required = &use_custom_options,
     .linked = &custom_saved.show_mirror_image,
     .number_type = SETTING_BYTE,
     .text = "Show mirror image",
     .explanation =
         "Show the kid's mirror image in the mirror.\n(default = true)"},
    {.id = SETTING_FALLING_EXIT_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.falling_exit_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Falling exit level",
     .explanation = "Level where the kid can progress to the next level by "
                    "falling off a specific room.\n(default = 6)"},
    {.id = SETTING_FALLING_EXIT_ROOM,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.falling_exit_room,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     .text = "Falling exit room",
     .explanation = "Room where the kid can progress to the next level by "
                    "falling down.\n(default = 1)"},
    {.id = SETTING_FALLING_ENTRY_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.falling_entry_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Falling entry level",
     .explanation = "If the kid starts in this level in this room, the "
                    "starting room will not be shown,\n"
                    "but the room below instead, to allow for a falling entry. "
                    "(default: level = 7, room = 17)"},
    {.id = SETTING_FALLING_ENTRY_ROOM,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.falling_entry_room,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     ...
     .explanation = "If the kid starts in this level in this room, the "
                    ...
                    "but the room below instead, to allow for a falling entry. "
                    "(default: level = 7, room = 17)"},
    {.id = SETTING_MOUSE_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mouse_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Mouse level",
     .explanation = "Level where the mouse appears.\n(default = 8)"},
    {.id = SETTING_MOUSE_ROOM,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mouse_room,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     .text = "Mouse room",
     .explanation = "Room where the mouse appears.\n(default = 16)"},
    {.id = SETTING_MOUSE_DELAY,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mouse_delay,
     .number_type = SETTING_WORD,
     .max = UINT16_MAX,
     .text = "Mouse delay",
     .explanation = "Number of seconds to wait before the mouse "
                    "appears.\n(default = 12.5)"},
    {.id = SETTING_MOUSE_OBJECT,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mouse_object,
     .number_type = SETTING_BYTE,
     .max = 255,
     .text = "Mouse object",
     .explanation =
    ...
    {.id = SETTING_MOUSE_START_X,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.mouse_start_x,
     .number_type = SETTING_BYTE,
     .max = 255,
     .text = "Mouse start X coordinate",
     .explanation =
         "Horizontal starting coordinate of the mouse.\n(default = 200)"},
    {.id = SETTING_LOOSE_TILES_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.loose_tiles_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Loose tiles level",
     .explanation =
         "Level where loose floor tiles will fall down.\n(default = 13)"},
    {.id = SETTING_LOOSE_TILES_ROOM_1,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.loose_tiles_room_1,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     .text = "Loose tiles room (1)",
     .explanation = "Rooms where visible loose floor tiles will fall "
                    "down.\n(default = 23, 16)"},
    {.id = SETTING_LOOSE_TILES_ROOM_2,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.loose_tiles_room_2,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 24,
     ...
     .explanation = "Rooms where visible loose floor tiles will fall "
                    "down.\n(default = 23, 16)"},
    {.id = SETTING_LOOSE_TILES_FIRST_TILE,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.loose_tiles_first_tile,
     .number_type = SETTING_BYTE,
     .max = 29,
     .text = "Loose tiles first tile",
     .explanation = "Range of loose floor tile positions that will be "
                    "pressed.\n(default = 22 to 27)"},
    {.id = SETTING_LOOSE_TILES_LAST_TILE,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.loose_tiles_last_tile,
     .number_type = SETTING_BYTE,
     .max = 29,
     .text = "Loose tiles last tile",
     .explanation = "Range of loose floor tile positions that will be "
                    "pressed.\n(default = 22 to 27)"},
    {.id = SETTING_JAFFAR_VICTORY_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.jaffar_victory_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Jaffar victory level",
     .explanation =
         "Killing the guard in this level causes the screen to flash, and "
         "event 0 to be triggered upon leaving the room.\n(default = 13)"},
    {.id = SETTING_JAFFAR_VICTORY_FLASH_TIME,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.jaffar_victory_flash_time,
     .number_type = SETTING_BYTE,
     .max = UINT16_MAX,
     .text = "Jaffar victory flash time",
     .explanation = "How long the screen will flash after killing "
                    "Jaffar.\n(default = 18)"},
    {.id = SETTING_HIDE_LEVEL_NUMBER_FIRST_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.hide_level_number_from_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Hide level number from level",
     .explanation = "First level where the level number will not be "
                    "displayed.\n(default = 14)"},
    {.id = SETTING_LEVEL_13_LEVEL_NUMBER,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.level_13_level_number,
     .number_type = SETTING_BYTE,
     .max = UINT16_MAX,
     .text = "Level 13 displayed level number",
     .explanation = "Level number displayed on level 13.\n(default = 12)"},
    {.id = SETTING_VICTORY_STOPS_TIME_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.victory_stops_time_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
     .text = "Victory stops time level",
     .explanation = "Level where Jaffar's death stops time.\n(default = 13)"},
    {.id = SETTING_WIN_LEVEL,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.win_level,
     .number_type = SETTING_WORD,
     .max = 16,
     .names_list = &never_is_16_list,
    ...
    {.id = SETTING_WIN_ROOM,
     ...
     .explanation = "Level and room where you can win the game.\n(default: "
                    "level = 14, room = 5)"},
    {.id = SETTING_LOOSE_FLOOR_DELAY,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.loose_floor_delay,
     .number_type = SETTING_BYTE,
     .min = 0,
     .max = 127,
     .text = "Loose floor delay",
     .explanation = "Number of seconds to wait before a loose floor "
                    "falls.\n(default = 0.92)"},
    {.id = SETTING_BASE_SPEED,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.base_speed,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 127,
     .text = "Base speed",
     .explanation = "Game speed when not fighting (delay between frames in "
                    "1/60 seconds). Smaller is faster.\n(default = 5)"},
    {.id = SETTING_FIGHT_SPEED,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.fight_speed,
     .number_type = SETTING_BYTE,
     .min = 1,
     .max = 127,
     .text = "Fight speed",
     .explanation = "Game speed when fighting (delay between frames in 1/60 "
                    "seconds). Smaller is faster.\n(default = 6)"},
    {.id = SETTING_CHOMPER_SPEED,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = &custom_saved.chomper_speed,
     .number_type = SETTING_BYTE,
     .min = 0,
     .max = 127,
     .text = "Chomper speed",
     .explanation = "Chomper speed (length of the animation cycle in frames). "
                    "Smaller is faster.\n(default = 15)"},
};

NAMES_LIST(level_type_setting_names, {
                                         "Dungeon",
                                         "Palace",
                                     });
KEY_VALUE_LIST(guard_type_setting_names, {{"None", -1},
                                          {"Normal", 0},
                                          {"Fat", 1},
                                          {"Skeleton", 2},
                                          {"Vizier", 3},
...
KEY_VALUE_LIST(off_setting_name,
               {{"Off", -1}});  // used for the seamless exit setting
...
setting_type level_settings[] = {
    ...
    {.id = SETTING_GUARD_TYPE,
     ...
     .max = 4,
     .text = "Guard type",
     .explanation = "Guard type used in this level (normal, fat, skeleton, "
                    "vizier, or shadow)."},
    {.id = SETTING_GUARD_HP,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
    ...
    {.id = SETTING_CUTSCENE,
     ...
     .explanation =
         "Cutscene that plays between the previous level and this level.\n"
         "0: none, 2 or 6: standing, 4: lying down, 8: mouse leaves,\n"
         "9: mouse returns, 12: standing or turn around"},
    {.id = SETTING_ENTRY_POSE,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = NULL,
     .number_type = SETTING_BYTE,
     .max = 2,
     .names_list = &entry_pose_setting_names_list,
     .text = "Entry pose",
     .explanation = "The pose the kid has when the level starts.\n"},
    {.id = SETTING_SEAMLESS_EXIT,
     .style = SETTING_STYLE_NUMBER,
     .required = &use_custom_options,
     .linked = NULL,
     .number_type = SETTING_SBYTE,
     .min = -1,
     .max = 24,
     .names_list = &off_setting_name_list,
     .text = "Seamless exit",
...
settings_area_type general_settings_area = {
    .settings = general_settings, .setting_count = COUNT(general_settings)};
settings_area_type gameplay_settings_area = {
    .settings = gameplay_settings, .setting_count = COUNT(gameplay_settings)};
settings_area_type visuals_settings_area = {
    .settings = visuals_settings, .setting_count = COUNT(visuals_settings)};
settings_area_type mods_settings_area = {.settings = mods_settings,
                                         .setting_count = COUNT(mods_settings)};
settings_area_type level_settings_area = {
    .settings = level_settings, .setting_count = COUNT(level_settings)};
...
bool is_mouse_over_rect(rect_type* rect) {
  return (mouse_x >= rect->left && mouse_x < rect->right &&
          mouse_y >= rect->top && mouse_y < rect->bottom);
}

// Maps the cursor position into a coordinate between (0,0) and (320,200) and
// sets mouse_x, mouse_y and mouse_moved.
void read_mouse_state() {
  ...
  int logical_scale_x =
      logical_width / 320;  // These may be higher than 1, if 4:3 aspect ratio
                            // scaling is enabled.
  int logical_scale_y = logical_height / 200;
  scale_x *= logical_scale_x;
  scale_y *= logical_scale_y;
  if (!(scale_x > 0 && scale_y > 0 && logical_scale_x > 0 &&
        logical_scale_y > 0))
    return;
  SDL_Rect viewport;
  SDL_RenderGetViewport(renderer_,
                        &viewport);  // Get the width/height of the 'black bars'
...
int controlled_area = 0;  // Whether the focus is on the left (0) or right (1)
                          // part of the screen in the Settings menu.
int next_setting_id = 0;  // For navigating up/down.
int previous_setting_id = 0;
bool at_scroll_up_boundary;  // When navigating up using keyboard/controller,
                             // whether we also need to scroll up
bool
    at_scroll_down_boundary;  // When navigating down using keyboard/controller,
                              // whether we also need to scroll down
...
void enter_settings_subsection(int settings_menu_id) {
  ...
  // Special case: for the level customization submenu, the linked variables
  // should depend on menu_current_level. So we need to initialize them now.
    ...
    for (int i = 0; i < settings_area->setting_count; ++i) {
      ...
      switch (setting->id) {
        ...
        case SETTING_CUTSCENE:
          setting->linked =
              &custom_saved.tbl_cutscenes_by_index[menu_current_level];
          break;
...
void draw_pause_menu_item(pause_menu_item_type* item, rect_type* parent,
                          int* y_offset, int inactive_text_color) {
    ...
    if (mouse_clicked) {
      if (is_mouse_over_rect(&selection_box)) {
        pause_menu_clicked(item);
               ...
               (drawn_menu == 0 || (drawn_menu == 1 && controlled_area == 0))) {
      pause_menu_clicked(item);
    }
  }
...
void draw_pause_menu() {
  ...
  draw_rect_with_alpha(
      &rect_bottom_text, color_0_black,
      0);  // Transparent so that the text "GAME PAUSED" is visible.
  ...
  for (int i = 0; i < COUNT(pause_menu_items); ++i) {
    draw_pause_menu_item(&pause_menu_items[i], &pause_rect_inner, &y_offset,
  ...
  switch (setting_id) {
    ...
    case SETTING_FULLSCREEN:
      ...
      SDL_SetWindowFullscreen(window_,
                              (new_state != 0) * SDL_WINDOW_FULLSCREEN_DESKTOP);
      break;
...
void decrease_setting(setting_type* setting, int old_value) {
  int new_value;
  if (setting->id == SETTING_JOYSTICK_THRESHOLD) {
    new_value = (((old_value + 999) / 1000) - 1) *
                1000;  // Nearest lower multiple of 1000.
  } else {
    new_value = old_value - 1;
  }
...
void draw_setting_explanation(setting_type* setting) {
  show_text_with_color(&explanation_rect, 0, -1, setting->explanation,
                       color_7_lightgray);
}
...
#define print_setting_value(setting, value) \
  print_setting_value_(setting, value, alloca(32), 32)
char* print_setting_value_(setting_type* setting, int value, char* buffer,
                           size_t buffer_size) {
  bool has_name = false;
...
void draw_setting(setting_type* setting, rect_type* parent, int* y_offset,
  ...
  if (setting->style == SETTING_STYLE_TOGGLE && !disabled) {
    ...
    // Toggling the setting: either by clicking on "ON" or "OFF", or by pressing
    // left/right.
    if (highlighted_setting_id == setting->id) {
      if (mouse_clicked) {
        if (!setting_enabled) {
    ...
    if (highlighted_setting_id == setting->id) {
      if (mouse_clicked) {
        rect_type right_hitbox = {setting_box.top, text_rect.right - 5,
                                  setting_box.bottom, text_rect.right + 10};
    ...
    if (highlighted_setting_id == setting->id) {
      int value_text_width = get_line_width(value_text, strlen(value_text));
      draw_image_with_blending(arrowhead_right_image, text_rect.right + 2,
                               text_rect.top);
      draw_image_with_blending(arrowhead_left_image,
                               text_rect.right - value_text_width - 6,
                               text_rect.top);
    }
    ...
    // show text only
    if (highlighted_setting_id == setting->id &&
        (setting->required == NULL || *(sbyte*)setting->required != 0)) {
      if (pressed_enter ||
          (mouse_clicked && is_mouse_over_rect(&setting_box))) {
...
void menu_scroll(int y) {
  settings_area_type* current_settings_area =
      get_settings_area(active_settings_subsection);
...
void draw_settings_area(settings_area_type* settings_area) {
  ...
  // We want to display which level we are editing, and start drawing slightly
  // lower.
  if (active_settings_subsection == SETTINGS_MENU_LEVEL_CUSTOMIZATION) {
    y_offset = 15;
    char level_text[16];
    snprintf(level_text, sizeof(level_text), "LEVEL %d", menu_current_level);
    show_text_with_color(&settings_area_rect, 0, -1, level_text,
                         color_15_brightwhite);
  }

  for (int i = 0;
       (i < settings_area->setting_count) && (num_drawn_settings < 9); ++i) {
    if (i >= scroll_position) {
      ++num_drawn_settings;
      draw_setting(&settings_area->settings[i], &settings_area_rect, &y_offset,
                   color_15_brightwhite);
    }
  }
  ...
  if (num_drawn_settings < settings_area->setting_count) {
    ...
    rect_type scrollbar_slider_rect = {
        .top = scrollbar_rect.top + scroll_position * scrollbar_height /
                                        settings_area->setting_count,
        .bottom = scrollbar_rect.top + (scroll_position + num_drawn_settings) *
                                           scrollbar_height /
                                           settings_area->setting_count,
        .left = scrollbar_rect.left,
        .right = scrollbar_rect.right};
    method_5_rect(&scrollbar_slider_rect, blitters_0_no_transp,
                  color_7_lightgray);
  }
}

void draw_settings_menu() {
  settings_area_type* settings_area =
      get_settings_area(active_settings_subsection);
  ...
  if (!have_mouse_input) {
    ...
    if (controlled_area == 0) {
      ...
      if (menu_control_y != 0) {
        // We check both directions in both cases, to scroll the highlighted row
        // back into sight even if the user scrolled it out of sight (with the
        // mouse wheel).
      ...
      highlighted_setting_id =
          current_settings_area->settings[highlighted_setting_index].id;
  ...
  for (int i = 0; i < COUNT(settings_menu_items); ++i) {
    ...
    int text_color = (highlighted_settings_subsection == item->id)
                         ? color_15_brightwhite
                         : color_7_lightgray;
    draw_pause_menu_item(&settings_menu_items[i], &pause_rect_inner, &y_offset,
                         text_color);
  }
...
void confirmation_dialog_result(int which_dialog, int button) {
  if (button == DIALOG_BUTTON_OK) {
    if (which_dialog == DIALOG_RESTORE_DEFAULT_SETTINGS) {
      ...
      turn_setting_on_off(SETTING_USE_INTEGER_SCALING, use_integer_scaling,
                          NULL);
  ...
  for (;;) {
    ...
    if (highlighted_button != old_highlighted_button) {
      ...
      uint32_t clear_color =
          SDL_MapRGBA(current_target_surface->format, 0, 0, 0, 255);
      ...
      show_text_with_color(&cancel_text_rect, 0, 0, "Cancel",
                           cancel_text_color);
      update_screen();
    }
...
void draw_select_level_dialog() {
  ...
  for (;;) {
    ...
    if (menu_current_level != old_edited_level_number) {
      ...
      shrink2_rect(&rect, &copyprot_dialog->text_rect, 2, 1);
      rect.bottom -= 14;
      show_text_with_color(&rect, 0, 0, "Customize level...",
                           color_15_brightwhite);
      clear_kbd_buf();
      ...
      draw_image_with_blending(arrowhead_left_image, 145 - 3,
                               input_rect.top + 3);
...
void draw_menu() {
  escape_key_suppressed =
      (key_states[SDL_SCANCODE_BACKSPACE] || key_states[SDL_SCANCODE_ESCAPE]);
  ...
  while (!need_close_menu) {
    ...
    if (is_menu_shown == 1) {
      is_menu_shown =
          -1;  // reset the menu if the menu is drawn for the first time
    ...
    if (have_mouse_input || have_keyboard_or_controller_input) {
      // The menu is updated+drawn within the same routine, so redrawing may be
      ...
      if (need_full_menu_redraw_count == 0) {
        SDL_Delay(1);
        continue;  // Don't redraw if there is no input to process (save CPU
                   // cycles).
      }
    }
...
void process_additional_menu_input() {
  ...
  have_keyboard_or_controller_input =
      (menu_control_x || menu_control_y || menu_control_back || pressed_enter);
  have_mouse_input = (mouse_moved || mouse_clicked ||
                      mouse_button_clicked_right || menu_control_scroll_y);
...
int key_test_paused_menu(int key) {
  ...
  if (is_joyst_mode) {
    ...
    int x_threshold =
        26000;  // Less sensitive, to prevent accidentally changing a setting.
    ...
    if (joy_x == 0 && joy_y == 0) {
      ...
      if (joy_xy_released) {
        needed_timeout_s =
            0.3f;  // The delay is longer for the first repetition.
    ...
    if (joy_AY_buttons_state == 0 && joy_B_button_state == 0) {
      ...
      if (joy_AY_buttons_state == 1 /* A pressed */) {
        ...
        joy_AY_buttons_state = 0;  // Prevent 'down' input being passed to the
                                   // controls if the game is unpaused.
...
#define process(x) \
  if (!process_func(rw, &(x), sizeof(x))) return
void process_ingame_settings_user_managed(SDL_RWops* rw,
                                          rw_process_func_type process_func) {
...
void process_ingame_settings_mod_managed(SDL_RWops* rw,
                                         rw_process_func_type process_func) {
...
void load_ingame_settings() {
  ...
  if (rw != NULL) {
    ...
    // This allows us not to worry about future and backward compatibility of
    // this file.
    calculate_exe_crc();
    ...
    if (exe_crc == expected_crc) {
      ...
      SDL_RWread(rw, &cfg_levelset_name_length,
                 sizeof(cfg_levelset_name_length), 1);
      SDL_RWread(rw, cfg_levelset_name, cfg_levelset_name_length, 1);
      //			printf("%s, %s\n", cfg_levelset_name,
      //levelset_name);
      process_ingame_settings_user_managed(
          rw, process_rw_read);  // Load the settings.
...
#define BINARY_8(b7, b6, b5, b4, b3, b2, b1, b0)                  \
  ((b0) | ((b1) << 1) | ((b2) << 2) | ((b3) << 3) | ((b4) << 4) | \
   ((b5) << 5) | ((b6) << 6) | ((b7) << 7))
#define BINARY_4(b7, b6, b5, b4) \
  (((b4) << 4) | ((b5) << 5) | ((b6) << 6) | ((b7) << 7))
#define _ 0
