...
void __pascal far check_collisions() {
  ...
  get_row_collision_data(collision_row, curr_row_coll_room,
                         curr_row_coll_flags);
  get_row_collision_data(collision_row + 1, below_row_coll_room,
                         below_row_coll_flags);
  get_row_collision_data(collision_row - 1, above_row_coll_room,
                         above_row_coll_flags);
...
void __pascal far move_coll_to_prev() {
  ...
  } else if (collision_row + 1 == prev_collision_row ||
...
void __pascal far check_bumped_look_right() {
  if ((Char.sword == sword_2_drawn ||
       Char.direction == dir_0_right) &&  // looking right
      is_obstacle_at_col(bump_col_left_of_wall)) {
    bumped(
        get_left_wall_xpos(curr_room, tile_col, tile_row) - char_x_right_coll,
        dir_FF_left);
  }
}
...
int __pascal far is_obstacle() {
  ...
  } else if (curr_tile2 == tiles_13_mirror && Char.charid == charid_0_kid &&
             Char.frame >= frame_39_start_run_jump_6 &&
             Char.frame < frame_44_running_jump_5 &&  // run-jump
...
void __pascal far bumped_floor(sbyte push_direction) {
  ...
  short seq_index;
  if (Char.sword != sword_2_drawn &&
      (word)(y_land[Char.curr_row + 1] - Char.y) >= (word)15) {
    bumped_fall();
  } else {
      ...
      if (Char.alive) {
        if (Char.sword == sword_2_drawn) {
          if (push_direction == Char.direction) {
            seqtbl_offset_char(
                seq_65_bump_forward_with_sword);  // pushed forward with sword
                                                  // (Kid)
            play_seq();
            ...
            seq_index =
                seq_64_pushed_back_with_sword;  // pushed back with sword
          }
        } else {
          frame = Char.frame;
          if (frame == 24 || frame == 25 || (frame >= 40 && frame < 43) ||
              (frame >= frame_102_start_fall_1 && frame < 107)) {
            seq_index =
                seq_46_hardbump;  // bump into wall after run-jump (crouch)
          } else {
  ...
  */
  ...
  loc_59E8:
    ...
    loc_59FB:
      ...
      if (tiletype == tiles_6_closer || tiletype == tiles_22_sword ||
...
void __pascal far check_gate_push() {
  ...
  short var_4;
  frame = Char.frame;
  if (Char.action == actions_7_turn || frame == frame_15_stand ||  // stand
...
void __pascal far check_guard_bumped() {
  if (Char.action == actions_1_run_jump && Char.alive < 0 &&
    ...
    if (
      ...
      if (is_obstacle()) {
        ...
        if (delta_x < 0 && delta_x > -13) {
          ...
          seqtbl_offset_char(
              seq_65_bump_forward_with_sword);  // pushed to wall with sword
                                                // (Guard)
          play_seq();
          load_fram_det_col();
        }
...
int __pascal far dist_from_wall_forward(byte tiletype) {
  ...
  if (tiletype == tiles_4_gate && !can_bump_into_gate()) {
    ...
    if (Char.direction < dir_0_right) {
      // looking left
      // return wall_dist_from_right[type] + char_x_left_coll -
      // coll_tile_left_xpos - 13;
      return char_x_left_coll -
             (coll_tile_left_xpos + 13 - wall_dist_from_right[type]);
    } else {
      // looking right
      return wall_dist_from_left[type] + coll_tile_left_xpos -
             char_x_right_coll;
